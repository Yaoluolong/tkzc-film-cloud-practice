    ...
    <div class="app-container">
         <el-form inline label-width="120px">
            <el-form-item label="电影券编码">
              <el-input v-model="query.couponStart" style="width:200px;" placeholder="输入电影券编码区间"></el-input>&emsp;--&emsp;<el-input v-model="query.couponEnd" style="width:200px;" placeholder="输入电影券编码区间"></el-input>
            </el-form-item>
            <el-form-item label="分发渠道">
              <el-input v-model="query.tag" style="width:200px;" placeholder="输入分发渠道"></el-input>              
            </el-form-item>
            <el-form-item label="发行影片">
              <el-input v-model="query.filmName" style="width:200px;" placeholder="选择影片名称查询"></el-input>     
            </el-form-item>              
            <el-form-item>
              <el-button type="primary" icon="el-icon-search"  @click="refreshTable">查询</el-button>
              <el-button type="danger" icon="el-icon-plus"  @click="exportData('getSaleCouponDetailList')">导出</el-button>
            </el-form-item>
         </el-form>
         ...
         <el-dialog title="电影券分发渠道标记" :visible.sync="dialogSignVisible" width="800px" height="400px">
          <el-form style="max-height:400px;overflow-y:auto;">
            <el-form-item label="销售订单号:" style="width:45%;display:inline-block;">{{saleCouponSign.orderNo}}</el-form-item>
            <el-form-item label="销售时间:" style="width:45%;display:inline-block;">{{saleCouponSign.saleTimeStr}}</el-form-item>
            <el-form-item label="电影券种类:" style="width:45%;display:inline-block;">{{saleCouponSign.typeName}}</el-form-item>
            ...
            <el-form-item label="电影券编码区间:">
              {{saleCouponSign.couponRange}}           
            </el-form-item>
            <el-form-item label="电影券分发渠道:">{{saleCouponSign.tagStr || '无'}}</el-form-item>
            <el-form-item label="" label-width="0px" style="width:100%;border-top:1px solid #000;font-weight:bold;margin-top:40px">
              <span style="">分发渠道:&nbsp;</span>
             <el-input v-model="tag" style="width:200px;margin-top:20px;" placeholder="输入电影券分发渠道" :disabled="isModify"></el-input>
         ...
         <el-dialog title="电影券使用汇总" :visible.sync="dialogDetailVisible" width="600px" height="400px" class="dialogDetailVisible">
          <el-form style="max-height:400px;overflow-y:auto;">
            ...
            <el-form-item label="结算总额:">{{saleCouponDetail.payMoney}}<span style="color:blue;">(订单总额不包含补差现金支付部分)</span></el-form-item>
            <el-form-item label="订单平均票价:">{{saleCouponDetail.priceAverage}}<span style="color:blue;">(实际交易总数除订单座位数)</span></el-form-item>
...
<script>
    import { getSaleCouponDetailList, getSaleCouponDetail, saleCouponSetTag, getSaleCouponDetailListExport } from '@/api/financeCenter'
    ...
    export default {
      ...
      methods: {
        ...
        refreshTable() {
          const cStart = this.query.couponStart ? this.query.couponStart.substring(0, 3) : ''
          const cEnd = this.query.couponEnd ? this.query.couponEnd.substring(0, 3) : ''
        ...
        exportData(func) {
          const result = this.$refs.table.getSelection().map(e => e.orderNo).join(',')
          if (!result) {
            exportData(getSaleCouponDetailListExport, { couponStart: this.query.couponStart, couponEnd: this.query.couponEnd, tag: this.query.tag, filmName: this.query.filmName })
          } else {
            exportData(getSaleCouponDetailListExport, { couponStart: this.query.couponStart, couponEnd: this.query.couponEnd, tag: this.query.tag, filmName: this.query.filmName, ids: result })
          }
        },
        openDetail(row) {
          this.$router.push({ path: '/finace_center/finance_report/coupon_used_order_detail', query: { orderNo: row.orderNo }})
        ...
        save() {
          saleCouponSetTag({ orderNo: this.saleCouponSign.orderNo, tag: this.tag }).then(res => {
