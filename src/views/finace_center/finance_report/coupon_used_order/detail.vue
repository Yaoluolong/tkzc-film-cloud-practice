  ...
  <div class="app-container">
    ...
    <el-form inline label-width="120px">
      ...
      <el-form-item label="放映制式" v-if="query.isUse==='1'">
        <remote-select
          v-model="query.copyType"
          clearable
          placeholder="请选择放映制式"
          action="/systemApi/couponOrder/getCopyType"
          style="width:200px;"
        ></remote-select>
      </el-form-item>
      <br />
      <el-form-item>
        ...
        <el-button
          type="danger"
          icon="el-icon-plus"
          @click="exportData('getSaleCouponOrderDetailListExport')"
          v-if="query.isUse==='1'"
        >导出</el-button>
        <el-button
          type="text"
        ...
        <el-button
          type="danger"
          icon="el-icon-plus"
          @click="exportData('getCouponOrderUnUseExport')"
          v-if="query.isUse==='0'"
        >导出</el-button>
      </el-form-item>
    ...
    >
      <el-form style="max-height:400px;overflow-y:auto;">
        ...
        <el-form-item label="结算总额:" v-if="saleCouponDetail.type!=='1'">
          {{saleCouponDetail.payMoney}}
          <span style="color:blue;">(结算总额不包含{{saleCouponDetail.type==='3'?'补差':'优惠'}}现金支付部分)</span>
        </el-form-item>
        ...
        <el-form-item label="订单平均票价:">
          ...
          <span style="color:blue;">(实际交易总额除订单座位数)</span>
        </el-form-item>
      </el-form>
      <div>
...
<script>
import {
  getSaleCouponOrderDetailList,
  getSaleCouponDetail,
  getSaleCouponOrderDetailListExport,
  getCouponOrderUnUseExport
...
export default {
  ...
  methods: {
    ...
    refreshTable() {
      const cLeft = this.query.couponCodeLeft
        ? this.query.couponCodeLeft.substring(0, 3)
        : ''
      const cRight = this.query.couponCodeRight
        ? this.query.couponCodeRight.substring(0, 3)
        : ''
    ...
    exportData(func) {
      if (func === 'getSaleCouponOrderDetailListExport') {
        exportData(getSaleCouponOrderDetailListExport, {
          tag: this.query.tag,
        ...
        exportData(getCouponOrderUnUseExport, {
          tag: this.query.tag,
