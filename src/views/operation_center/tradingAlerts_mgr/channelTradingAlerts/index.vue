  ...
  <div class="app-container">
    ...
    <el-form inline label-width="120px">
      ...
      <el-form-item label="预警状态" v-if="query.checkStatus ==='1'">
        ...
        <remote-select
          v-model="query.warnStatus"
          clearable
          showAllOption
          placeholder="请选择电影券类型"
          action="/systemApi/dict/getList"
          :query="{type:'platformWarnStatus'}"
        ></remote-select>
      </el-form-item>
      <el-form-item>
        <el-button
        ...
        <el-button
          type="primary"
          icon="el-icon-search"
          @click="refreshCheckAccountTable"
    ...
    >
      ...
      >
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="$router.push({path:'/operation_center/paltform_trading_alerts_mgr/view_account_detail',query:{channelId:row.channelId}})"
          >{{row.name}}</el-button>
        </template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column width="80" label="启用状态" align="center" prop="isWarning" fixed="right">
        <template slot-scope="{row}">
          <switch-confirm
            v-model="row.isWarning"
            confirm-text="是否进行此操作"
            valueName="isWarning"
            isRefreshTable
            idKey="channelId"
            :id="row.channelId"
            action="/systemApi/platformWarn/setStatus"
            @submitChange="refreshTable"
          ></switch-confirm>
        </template>
      </el-table-column>
      <el-table-column min-width="250" label="配置预警" align="center" prop="oper" fixed="right">
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="isPass('1',row.channelId , {channelId:row.channelId}, 'channel_trading')"
            :disabled="row.isWarning==='1'"
          >{{row.isSetWarning==='1'? '已设置':'未设置'}}</el-button>
        </template>
      </el-table-column>
    </page-table>
    <page-table
      ref="checkAccountTable"
      index
      :query="query"
      :fetch="checkAccountTable"
      v-show="query.checkStatus ==='1'"
    >
      <el-table-column
        min-width="100"
        label="商家名称"
      ...
      >
        <template slot-scope="{row}">
          ...
          >{{row.warnStatus==='0'? '请确认':(row.warnStatus==='1'?'--':(row.warnStatus==='2'?'请对账':'--'))}}</el-button>
          <el-button
            type="text"
...
<script>
import {
  getPlatformWarnPageList,
  getCheckAccountPageList,
  checkPlatformWarnAuthority
...
export default {
  ...
  methods: {
    ...
    isPass(type, channelId, query, tableName) {
      checkPlatformWarnAuthority({ type: type, channelId: channelId })
        .then(res => {
          this.$router.push({
            path: '/operation_center/paltform_trading_alerts_mgr/' + tableName,
            query: query
          })
        })
        .catch(e => {
