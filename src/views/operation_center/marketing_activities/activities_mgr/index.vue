  ...
  <div class="app-container">
    <el-form inline label-width="120px">
      ...
      <el-form-item>
        ...
        <el-button
          type="danger"
          icon="el-icon-plus"
          v-if="query.approvalStatus!=='2'"
          @click="$router.push({path:'/operation_center/marketing_activities/activities_mgr/create'})"
        >新增</el-button>
        <el-button
          type="success"
    ...
    <page-table ref="table" :query="query" :fetch="queryTable">
      ...
      <el-table-column min-width="180" label="操作" align="center" prop="prev" fixed="right">
        <template slot-scope="{row}">
          ...
          <el-button
            type="text"
            @click="$router.push({path:'/operation_center/marketing_activities/activities_Detail',query:{id:row.id}})"
            v-if="query.approvalStatus!=='3' && !row.canReturn"
          >活动明细</el-button>
          <el-button
            type="text"
            @click="activitySetBack(row)"
            v-if="query.approvalStatus==='1' && row.canReturn"
          >活动撤档</el-button>
          <el-button
            type="text"
            @click="gotoViewDetails(row)"
            v-if="query.approvalStatus==='2'"
          >活动详情</el-button>
          <el-dropdown type="text" v-if="query.approvalStatus==='1'">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item
                @click.native="isTopping(row)"
                v-if="row.unStart"
              >{{row.isTop==='0'?'置顶':'取消置顶'}}</el-dropdown-item>
              <el-dropdown-item
                @click.native="isStopping(row)"
                v-if="row.unStart"
              >{{row.isStop==='0'?'暂停':'恢复'}}</el-dropdown-item>
          ...
          <span v-if="query.approvalStatus==='3' && row.showStatus===1">
            ...
            <el-dropdown type="text">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item
                  @click.native="$router.push({path:'/operation_center/marketing_activities/view_activities_details',query:{id:row.id}})"
                  v-if="row.auditStatus==='4'"
                >退回原因</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </span>
          <span v-if="query.approvalStatus==='3' && row.showStatus===2">
            <el-button
              type="text"
              @click="$router.push({path:'/operation_center/marketing_activities/view_activities_details',query:{id:row.id}})"
            >查看</el-button>
          </span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="补充活动款" :visible.sync="fillUpVisible" width="800px" :show-close="false" left>
      ...
      >
        ...
        <div
          v-if="!(fillUpParams.weixinPay instanceof Array) && fillUpParams.weixinPay && fillUpParams.weixinPay.activityConfig"
        >
          <h3>微信支付</h3>
          <div v-for="(item,index) in fillUpParams.weixinPay.activityConfig" :key="index">
            <el-form-item label="活动可用制式：">{{item.copyType}}</el-form-item>
            <el-form-item
              label="活动补贴消耗上限："
              :prop="'weixinPay.activityConfig.'+index+'.ceiling'"
...
import {
  ...
  getActivityOrderExport,
  activitySetBack,
  getActivityPayConfig,
  addActivityFee
...
export default {
  ...
  data() {
    return {
      ...
      fullUpRules: {
        ...
        'activityInfo.addFeeContent': {
          required: true,
          message: '请填写补充活动款金额说明',
          trigger: 'blur'
        }
      }
    }
  ...
  methods: {
    ...
    gotoViewDetails(row) {
      this.$router.push({
        path: '/operation_center/marketing_activities/view_activities_details',
        query: { id: row.id }
      })
    },
    // 是否置顶
    ...
    isStopping(row) {
      ...
      const isStopTip2 =
        row.isStop === '1'
          ? '活动恢复后将继续进行，确认恢复活动？'
          : '活动暂停将暂时停止活动，确认暂停活动？'
      this.$confirm(isStopTip2, '下架提示', { type: 'warning' }).then(
        async() => {
    ...
    // 提交审核
    gotoApproval(row) {
      activitySetApprovalStatus({ id: row.id, approvalStatus: '1' }).then(
        res => {
          this.$message({
            type: 'success',
            message: `提交审核成功!`
    ...
    activitiesEdit(row) {
      this.$router.push({
        path: '/operation_center/marketing_activities/activities_mgr/edit',
        query: { id: row.id }
      })
    },
    // 删除
    activitiesDel(row) {
      this.$confirm(`删除后活动记录将从列表删除，确认删除活动？`, '删除提示', {
        type: 'warning'
      }).then(async() => {
    ...
    activityDown(row) {
      ...
      this.$confirm(`活动下架后不可恢复，确认下架活动？`, '下架提示', {
        type: 'warning'
      }).then(async() => {
    ...
    activitySetBack(row) {
      // 活动撤档
      this.$confirm(`活动撤档后需要重新提交审核，确认撤档？`, '温馨提示', {
        type: 'warning'
      }).then(async() => {
    ...
    submitAudit() {
      ...
      this.$refs.fillUpForm.validate(valid => {
        if (valid) {
          this.$confirm(
            `补充活动款完成后，请通知财务对进行确认收款的操作，收款后活动上限将根据设置金额生效并自动恢复活动`,
            '温馨提示',
            { type: 'warning' }
          ).then(async() => {
