<template>
	<div class="">    
	    <el-form label-width="140px" :model="params" :rules="bankSubsidyRules" ref="bankSubsidyRulesForm">
        ...
        <el-form-item label="白名单号码:" prop="key" v-if="params.hasWhiteList==='1'">
          <file-upload v-model="params.key" @fileUploadKey="fileUploadKey" content="文件大小不超过3M，建议单次导入号码10万个以内"></file-upload>
        </el-form-item>
        ...
        <el-form-item label-width="200px" label="每笔订单最多可购座位数:" prop="orderMaxCanBuy" v-if="params.orderSeatsLimit==='1'">
          <el-input v-model="params.orderMaxCanBuy" style="width:320px;" placeholder="输入每笔订单最多可购座位数"></el-input>
        </el-form-item>
        ...
        <el-form-item label-width="200px" label="每个活动日限购座位总张数:" prop="totalMaxCanBuy" v-if="params.totalSeatsLimit==='1'" style="width:450px;">
          <tip content="单个活动日限制的总座位数量张数">
            <el-input v-model="params.totalMaxCanBuy" placeholder="每个活动日限购座位总张数"></el-input>
          </tip>
        </el-form-item>
        ...
        <!-- 支付方式配置 -->
        <el-form-item label="" label-width="0px" prop="weixinPay" v-if="params.payType && params.payType.includes('1')">
          <weixin-pay ref="weixinPay" v-model="params.weixinPay" :cinemaGroupId="cinemaGroupId"></weixin-pay>
        </el-form-item>
        
        <el-form-item label="" label-width="0px" prop="unionPay" v-if="params.payType && params.payType.includes('2')">
        ...
        <el-form-item label="用户参与规则:"  class="noTag" style="width:600px;" prop="allCount">
            单个用户参与总次数&emsp;&emsp;<el-input placeholder="次数" v-model.trim="params.ruleConfig.allCount" style="width:120px"></el-input>
        </el-form-item>
        <el-form-item label=" "  class="noTag" style="width:600px;" prop="dayCount">
            单个用户每日参与次数&emsp;<el-input placeholder="次数" v-model.trim="params.ruleConfig.dayCount" style="width:120px"></el-input>
        </el-form-item>
        <el-form-item label=" "  class="noTag" style="width:600px;" prop="weekCount">
            单个用户每周参与次数&emsp;<el-input placeholder="次数" v-model.trim="params.ruleConfig.weekCount" style="width:120px"></el-input>             
        </el-form-item>
        <el-form-item label=" "  class="noTag" style="width:600px;" prop="monthCount">
            单个用户每月参与次数&emsp;<el-input placeholder="次数" v-model.trim="params.ruleConfig.monthCount" style="width:120px"></el-input>             
        </el-form-item>
...
export default{
  name: 'bank_subsidy',
  components: { ChannelSelector, CityCascader, FilmSelector, FileUpload, WeixinPay, UnionPay },
  props: {
    value: Object,
    activityType: String
  },
  ...
  data() {
    ...
    const checkKey = (rule, value, cb) => {
      if (this.params.hasWhiteList === '1' && !this.params.key && !this.$route.query.id) {
        cb(new Error('请上传白名单'))
      }
      cb()
    }
    const checkWeixinPay = (rule, value, cb) => {
      if (this.params.payType.includes('1')) {
        this.params.weixinPay.activityConfig.forEach(e => {
          if (e.tag.length === 0 || e.copyType.length === 0 || e.ceiling.length === 0) {
    ...
    const checkUnionPay = (rule, value, cb) => {
      const userLimit1 = Object.keys(this.params.unionPay.activityConfig.useLimit)[0]
      if (this.params.payType.includes('2')) {
        if (this.params.unionPay.payConfigId.length === 0 || this.params.unionPay.copyType.length === 0 || this.params.unionPay.ceiling.length === 0) {
          cb(new Error('请填写完整支付配置'))
        }
        if (userLimit1 === 'undefined' || !this.params.unionPay.activityConfig.useLimit[userLimit1]) {
          cb(new Error('请填写完整支付配置'))
        }
        if (this.params.unionPay.discountType === '1' && !this.params.unionPay.activityConfig.money) {
          cb(new Error('请填写完整支付配置'))
        }
        if (this.params.unionPay.discountType === '2' && (!this.params.unionPay.activityConfig.interval1 || !this.params.unionPay.activityConfig.interval2)) {
          cb(new Error('请填写完整支付配置'))
        }
        if (this.params.unionPay.discountType === '3' && (!this.params.unionPay.activityConfig.scale || !this.params.unionPay.activityConfig.limit)) {
          cb(new Error('请填写完整支付配置'))
        }
      }
      cb()
    ...
    return {
      ...
      bankSubsidyRules: {
        ...
        key: { required: true, validator: checkKey, message: '请上传白名单', trigger: 'change' },
        payType: { required: true, message: '请选择支付方式', trigger: 'change' },
        orderMaxCanBuy: { required: true, message: '请输入每笔订单最多可购座位数', trigger: 'blur' },
        totalMaxCanBuy: { required: true, message: '请输入日限购座位总张数', trigger: 'blur' },

        weixinPay: { required: true, validator: checkWeixinPay, message: '请填写完整支付配置', trigger: 'blur' },
        unionPay: { required: true, validator: checkUnionPay, message: '请填写完整支付配置', trigger: 'blur' }
  ...
  async created() {
    if (this.$route.query.id && this.value) {
      ...
      this.$nextTick(el => {
        ...
        typeof (this.params.payType) === 'string' && this.$set(this.params, 'payType', this.params.payType.split(',').filter(e => e))
        typeof (this.params.devices) === 'string' && this.$set(this.params, 'devices', this.params.devices.split(',').filter(e => e))
        typeof (this.params.scopes) === 'string' && this.$set(this.params, 'scopes', this.params.scopes.split(',').filter(e => e))
      })
