<template>
  <div class>
    <el-form
      label-width="140px"
      :model="params"
      :rules="platformSubsidyRules"
      ref="platformSubsidyRulesForm"
    >
      ...
      <div v-if="params.cinemaType">
        ...
        <el-form-item
          label="影片固定售价:"
          prop="filmConfig.money"
          style="width:300px;"
          v-if="params.filmConfig.type==='1'"
        >
          <el-input clearable v-model.trim="params.filmConfig.money" style="width:100px"></el-input>&emsp;元
        </el-form-item>
        <el-form-item
          label="影片立减金额:"
          prop="interval"
          style="width:500px;"
          v-if="params.filmConfig.type==='2'"
        >
          <el-input clearable v-model.trim="params.filmConfig.interval1" style="width:100px"></el-input>&emsp;至&emsp;
          <el-input clearable v-model.trim="params.filmConfig.interval2" style="width:100px"></el-input>&emsp;元
        </el-form-item>
        ...
        <el-card style="margin-top:10px">
          ...
          <el-form-item style="width:1021px" prop="payTypeDetail" v-show="activeName==='app'">
            <ul class="ul">
              <el-checkbox-group
                v-model="params.payTypeDetail.app"
                @change="checkedItemChange(payTypeList['app'])"
              >
                <li class="ul-li">
                  <el-checkbox
                    @change="slelectAllPayType(payTypeList['app'])"
                    :label="'-1'"
                    v-model="appCheckAll"
                  >全选</el-checkbox>
                </li>
          ...
          <el-form-item style="width:1021px" prop="payTypeDetail1" v-show="activeName==='wap'">
            <ul class="ul">
              <el-checkbox-group
                v-model="params.payTypeDetail.wap"
                @change="checkedItemChange(payTypeList['wap'])"
              >
                <li class="ul-li">
                  <el-checkbox
                    @change="slelectAllPayType(payTypeList['wap'])"
                    :label="'-1'"
                    v-model="wapCheckAll"
                  >全选</el-checkbox>
                </li>
...
export default {
  ...
  data() {
    ...
    const checkInterval = (rule, value, cb) => {
      if (
        this.params.filmConfig.type === '2' &&
        (this.params.filmConfig.interval1.length === 0 ||
          this.params.filmConfig.interval2.length === 0)
      ) {
    ...
    return {
      ...
      payTypeList: {
        app: [{ name: '', value: '', payTypeList: [] }],
        wap: [{ name: '', value: '', payTypeList: [] }]
      },
      activeName: 'wap',
      appCheckAll: false,
      wapCheckAll: false,
      ...
      platformSubsidyRules: {
        ...
        payTypeDetail: {
          required: true,
          validator: checkPayDetail,
          message: '请确认微信H5和移动APP支付方式是否都已选择',
          trigger: 'blur'
        },
        payTypeDetail1: {
          required: true,
          validator: checkPayDetail1,
          message: '请确认微信H5和移动APP支付方式是否都已选择',
          trigger: 'blur'
        }
  ...
  methods: {
    ...
    checkedItemChange(row) {
      ...
      if (
        ...
        this.params.payTypeDetail.app.splice(
          this.params.payTypeDetail.app.findIndex(el => el === '-1'),
          1
        )
      }
      ...
      if (
        ...
        this.params.payTypeDetail.wap.splice(
          this.params.payTypeDetail.wap.findIndex(el => el === '-1'),
          1
        )
      }
