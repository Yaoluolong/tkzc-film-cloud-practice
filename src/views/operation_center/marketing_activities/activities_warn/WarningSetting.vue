<template>
  <div class="app-container">
    ...
    <el-form ref="form" label-width="140px" style="width:1000px;" :model="params" :rules="rules">
      <el-form-item label="活动ID：">{{params.id}}</el-form-item>
      <el-form-item label="活动名称：">{{params.name}}</el-form-item>
      <el-form-item label="活动补贴总金额：">{{params.totalAmount}}</el-form-item>
      ...
      <div class="triggerWarningConditions">
        <div v-for="(item, index) in params.warnConfig" class="siggleConditions" :key="index">
          ...
          <el-form-item
            label="触发预警条件："
            :prop="'warnConfig.'+index+'.warnPercent'"
            ...
            <div class="ruleHeader">
              <div class="oper" style="float:right;">
                <i
                  ...
                  v-if="params.warnConfig.length!=1"
                ></i>
                <i
                  class="el-icon-circle-plus danger"
                  title="添加"
                  @click="newActivityRule"
                  v-if="index==params.warnConfig.length-1"
                ></i>
              </div>
            </div>
          </el-form-item>
          <el-form-item
            label="预警处理方式："
            :prop="'warnConfig.'+index+'.dealType'"
      ...
      <el-form-item label="活动账单：">
        <span>自动生成活动账单</span>&emsp;&emsp;&emsp;&emsp;
        <span style="color:rgb(168, 174, 187);">每次触发预警时，将自动生产一条活动对账单</span>
      </el-form-item>
      ...
      <el-form-item label="模板类型：" prop="smsTemplateType">
        <remote-select
          v-model="params.smsTemplateType"
          clearable
          placeholder="请选择模板类型"
          action="/systemApi/dict/getList"
          :query="{type:'smsTemplateType'}"
          style="width:230px;"
          @change="smsTemplateTypeChange"
        ></remote-select>
      </el-form-item>
      ...
      <el-form-item label="短信内容：" prop="smsTemplateId" v-if="params.smsTemplateStr">
        <el-table border align="center" :data="smsContent">
          <el-table-column prop="content" label="短信模板内容" header-align="center"></el-table-column>
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        warnConfig: [
          {
            required: true,
            message: '请填写预警条件和处理方式',
            trigger: 'blur'
          },
          {
            validator: checkWarnConfig,
            message: '请填写预警条件和处理方式',
            trigger: 'blur'
          }
        ],
  ...
  methods: {
    ...
    templateStrChange() {
      smsTemplateGetList({
        smsType: this.params.smsType,
        parentType: this.params.smsTemplateType,
        typeStr: this.params.smsTemplateStr
      }).then(res => {
        this.smsContent = res
      })
    },
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          this.$confirm(
            `预警设置完成后，请通知财务对预警内的合作放预付额度进行确认收款的操作，否则预警条件无法开始执行！`,
            '执行提示',
            { type: 'warning' }
          ).then(() => {
            activityWarnUpdate(this.params)
              .then(res => {
                this.closeTab(true)
                this.$router.push(
                  '/operation_center/marketing_activities/activities_warn'
                )
              })
              .catch(e => {
