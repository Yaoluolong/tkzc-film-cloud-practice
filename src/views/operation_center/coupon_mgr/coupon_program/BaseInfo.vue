<template>
  <div class="app-container">
    <el-form label-width="120px"  style="width:1200px;" :model="params" :rules="rules" ref="baseInfo">      
      ...
      <el-form-item label="销售时规则是否允许修改" prop="isAllowUpdate" label-width="200">
        ...
        <span style="font-size:14px;color:rgb(142, 142, 142);margin-left:8px;">业务员在销售电影券时选择该电影券规则模版后是否允许让其进行修改，在创建模版的时候默认是“不允许”！</span>
      </el-form-item>
      ...
      <el-form-item label="可用商家" prop="channelId" >
        <template v-if="ok || !$route.query.id">
          <channel-selector ref="channelComponent" v-model="params.channelList" type="1" :changeFun="channelChangeFun"></channel-selector> 
        </template>
      </el-form-item>
      <el-form-item label="可用影院" prop="cinemaGroupId" >
        <remote-select v-model="params.cinemaGroupId" clearable placeholder="请选择可用影院" action="/systemApi/cinemaGroup/getList" :query="{}" style="width:230px;"></remote-select>
      </el-form-item>      
      <div v-for="(rule,index) in params.ruleList" :key="rule.key" v-if="params.cinemaGroupId">
        ...
        <el-form-item label="选择影厅："  :prop="'ruleList.'+index+'.hallType'" :rules="programInfoRules.hallType" v-if="params.cinemaGroupId && rule.settlementType === '1'" style="width:300px;">
           <tip content="不同类型的影厅可以在影厅管理页面进行配置！">
            <remote-select v-model="rule.hallType" placeholder="请选择影厅" action="/systemApi/dict/getList"  :query="{type:'hallType'}"></remote-select>
           </tip>
        </el-form-item>
        <el-form-item label="选择制式：" :prop="'ruleList.'+index+'.standard'" :rules="programInfoRules.standard" v-if="params.cinemaGroupId && rule.settlementType === '2'">
              <standard-selector ref="standard" v-model="rule.standard" :cinemaGroupId="params.cinemaGroupId"></standard-selector>
        </el-form-item>
        <el-form-item label="选择影片：" :prop="'ruleList.'+index+'.filmId'"  :rules="programInfoRules.filmId" v-if="rule.settlementType === '3'">
          ...
          <film-selector :content-list="rule.filmList" v-if="rule.filmType === '1'" getFilmId v-model="rule.filmId"></film-selector>
        </el-form-item>
        <el-form-item label="电影券种类：" prop="couponType">
          <remote-select v-model="params.couponType" clearable placeholder="请选择电影券种类" action="/systemApi/dict/getList" :query="{type:'couponType'}" style="width:230px;"></remote-select>
        </el-form-item>
        ...
        <el-form-item label="补差类型：" v-if="rule.settlementType" :prop="'ruleList.'+index+'.diffType'" :rules="programInfoRules.diffType" style="width:350px;">
          <tip content="默认值是固定补差，可以根据实际情况更改未差额补差！">            
            <el-radio v-model="rule.diffType" label="2" @change="complementChange(rule.diffType, rule.settlementType)">固定补差</el-radio>
            <el-radio v-model="rule.diffType" label="1" >差额补差</el-radio>
          </tip>
        </el-form-item>

        <el-form-item label=" " label-width="140px" v-if="(rule.diffType==='1' && rule.settlementType)">
          超出此范围需补差额根据影票价格减去电影券最大可兑换价值
        </el-form-item>

        <el-form-item  label="设定补差：" v-if="rule.diffType==='2'&& rule.settlementType==='1'">
          <hall-complement v-model="rule.diffRule" ref="complementHallRef" getHall :outDataId="rule.hallType" @diffRuleChange="diffByValue" ></hall-complement>
        </el-form-item>
        <el-form-item v-if="rule.diffType==='2'&& rule.settlementType==='2'">
          <standard-complement v-model="rule.diffRule" ref="complementStandardRef" getStandard :outDataId="rule.standard" :cinemaGroupId="params.cinemaGroupId" @diffRuleChange="diffByValue"></standard-complement>        
        </el-form-item>
        <el-form-item v-if="rule.diffType==='2'&& rule.settlementType==='3'">
          <film-complement v-model="rule.diffRule" ref="complementRef" getFilm :filmType="rule.filmType" :outDataId="rule.filmId" :outDataList="rule.filmList" @diffRuleChange="diffByValue"></film-complement>          
        </el-form-item>
        </div>        
      </div>
    </el-form>
...
import { getSettlementPriceCinemaHallList, getProgramCopyType } from '@/api/mallCenter'
...
export default {
  components: { CouponSelector, ChannelSelector, CinemaSelector, DateSelector, HallSelector, StandardSelector, FilmSelector, FilmComplement, StandardComplement, HallComplement },
  props: {
    outSideData: Object
  },
  data() {
    return {
      params: this.outSideData ? this.outSideData : {
        // interfaceTypeId: '',
        couponType: '',
        isAllowUpdate: '2',
        channelId: '',
      ...
      rules: {
        ...
        name: { required: true, max: 20, message: '请输入方案名称,最多20个字', trigger: 'blur' },
        dateLimitType: { required: true, message: '请选择券不可用时段', trigger: ['blur', 'change'] },
        time: { required: true, message: '请选择规则时间', trigger: ['blur', 'change'] },
        channelId: { required: true, message: '请选择可用商家', trigger: 'blur' },
        cinemaId: { required: true, message: '请选择可用影院', trigger: 'blur' },
        couponType: { required: true, message: '请选择执行电影券种类', trigger: 'change' },
        couponStyle: { required: true, message: '请选择执行电影券类型', trigger: 'change' }
  ...
  methods: {
    ...
    cinemaChangeFun(cinema, callBack) {
      if (!cinema.checked) {
        this.$confirm('规则内包含此影院的信息,删除此影院后将删除关联信息', '提示', { type: 'warning' }).then(() => {
          this.params.ruleList = [{
            filmId: '-1',
            filmType: '-1',
            hallId: '',
    ...
    async cinemaDelFun(cinema, index) {
      ...
      if (flag) {
        ...
        this.$confirm('规则内包含此影院的信息,删除此影院后将删除关联信息', '温馨提示', { type: 'warning' }).then(() => {
          this.deleteRuleInfoForCinema(cinema)
          this.$refs.cinemaComponent.handleClose(index)
          this.$forceUpdate()
        })
