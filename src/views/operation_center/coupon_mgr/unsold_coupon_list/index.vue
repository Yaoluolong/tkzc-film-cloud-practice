<template>
  <div class="app-container">
    ...
    <page-table ref="table" index :query="query" :fetch="queryTable">
      ...
      <el-table-column min-width="250" label="操作" align="center" prop="oper" fixed="right">
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="$router.push({path:'/operation_center/coupon_mgr/unsold_coupon_detail',query:{orderNo:row.orderNo}})"
          >查看</el-button>
    ...
    <el-dialog title="生成电影券" :visible.sync="dialogFormVisible" width="40%">
      <el-form label-width="120px" :model="params" :rules="rules" ref="form">
        ...
        <el-form-item label="生成数量" prop="num">
          <el-input v-model="params.num" value="0" :min="1" label="描述文字" style="width:200px"></el-input>&nbsp;&nbsp;张
          <span style="margin-left: 8px;color:#8e8e8e;" size="medium">只能是数值，且必须大于0</span>
        </el-form-item>
      </el-form>
    ...
    <el-dialog
      ...
      width="30%"
    >
      <div slot="title" class="el-dialog-title">服务器正在努力生成卡，请稍后...</div>
      <span style="text-align:center">
        <el-progress :percentage="percentage"></el-progress>
      </span>
      <span slot="footer" class="el-dialog-footer">生成卡的速度取决于销售订单中出售数量，请耐心等候！</span>
    </el-dialog>
    <!-- 导出券 -->
    <el-dialog title="导出券" :visible.sync="exportFormVisible" width="40%">
      <el-form label-width="120px" :model="agur" :rules="exportRules" ref="exportForm">
        <el-form-item label="电影券批次号" prop="batchNo">{{agur.batchNo||'--'}}</el-form-item>
        <el-form-item label="电影券数量" prop="num">{{agur.num||'--'}}张</el-form-item>
        <el-form-item label="销售订单号" prop="saleOrderNo">
          <el-input clearable v-model="agur.saleOrderNo" label="输入销售订单号" style="width:200px"></el-input>
          <span class="db ml8" style="color:#8e8e8e;" size="medium">销售订单号未输入则导出整个批次！</span>
        </el-form-item>
...
import {
  getUnsoldOrderPageList,
  createUnsoldOrder,
  countUnsoldCouponPercent,
  exportCouponDetail,
