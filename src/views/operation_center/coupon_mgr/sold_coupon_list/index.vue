  ...
  <div class="app-container">
    <el-form inline label-width="120px">
      ...
      <el-form-item label="电影券号码">
        <el-input
          ...
          clearable
          placeholder="输入电影券编码区间查询"
          style="width:230px;"
        ></el-input>&nbsp;--&nbsp;
        <el-input
          v-model="query.couponEnd"
          clearable
          placeholder="输入电影券编码区间查询"
          style="width:230px;"
        ></el-input>
      ...
      <el-form-item>
        ...
        <el-button
          ...
          @click="$router.push({path:'/operation_center/coupon_mgr/sold_coupon_list/create',query:{}})"
        >创建销售单</el-button>
      </el-form-item>
    </el-form>
      ...
      >
        <template slot-scope="{row}">
          <div v-if="row.status !== '2'">
            <el-button
              type="text"
              @click="$router.push({path:'/operation_center/coupon_mgr/sold_coupon_list/edit',query:{orderNo:row.orderNo}})"
              v-if="query.checkStatus === '0'"
            >编辑/提交审核</el-button>
            <el-button
              type="text"
              @click="cancelSoldCouponOrder(row.orderNo)"
              v-if="query.checkStatus === '0'"
            >撤销订单</el-button>
            <el-button
              type="text"
              @click="$router.push({path:'/operation_center/coupon_mgr/get_sale_coupon_list',query:{orderNo:row.orderNo}})"
              v-if="query.checkStatus !== '0'"
            >销售单明细</el-button>
            <el-button
              type="text"
            ...
            <el-button
              type="text"
              @click="cancelSoldCouponOrder(row.orderNo)"
              v-if="row.checkStatus === '1' && query.checkStatus === '1'"
            >撤销销售</el-button>
    ...
    <el-dialog title="导出券" :visible.sync="exportFormVisible" width="50%" class="abow_dialog">
      <el-form label-width="140px" :model="agur" :rules="exportRules" ref="exportForm">
        <el-form-item label="电影券销售订单号" prop="batchNo">{{agur.orderNo||'--'}}</el-form-item>
        <el-form-item label="电影券数量" prop="num">{{agur.num||'--'}}张</el-form-item>
        ...
        <el-form-item label="是否一券多码" prop="isMult">
          ...
          <div v-if="agur.isMult==='1'">
            ...
            <el-input
              ...
              style="width:100px;"
              :disabled="agur.hasExport==='1'"
            ></el-input>&emsp;张电影券合成一个新的电影券以及新的密码二维码。
          </div>
          <span
            v-if="agur.isMult==='1'"
            style="color:#d6d5d5;margin-top:10px;line-height:20px;display: block;"
          >导出券的时候在已选取导出字段基础上，额外自动也会导出每张券的新绑定密码以及新的密码二维码，根据填写的券数量这些券的新绑定密码以及新的密码二维码均一致，同时执行了一码多券并导出后，第二次就不可再进行修改，请慎重！</span>
        </el-form-item>
...
import {
  getCouponOrderPageList,
  cancelSoldCouponOrder,
  couponOrderExportCouponDetail,
  getUnsoldExportCouponField
...
export default {
    ...
    return {
      ...
      exportRules: {
        ...
        isMult: [
          ...
          {
            validator: checkMoreCode,
            message: '请填写正确多码券数,且券数不能大于电影券数量',
            trigger: 'blur'
          }
        ],
        mobile: [
  ...
  methods: {
    ...
    cancelSoldCouponOrder(orderNo) {
      this.$confirm(
        `撤销销售将对电影券销售订单进行撤单操作,并将销售订单中电影券还原回未销售状态,是否开始执行?`,
        '温馨提示',
        { type: 'warning' }
      )
        .then(async() => {
    ...
    fieldChange(val) {
      ...
      this.selectAll =
        this.selectedFieldList.length === this.agur.fieldList.length
    },
    exportData(row) {
      ...
      this.$refs.exportForm.validate(flag => {
        if (flag) {
          ...
          if (temp.isMult === '1' && +this.agur.num % +temp.couponNum > 0) {
            this.$confirm(
              '当前填写的数值与电影券销售总量存在不整除的情况，如果继续则最后剩多少就整合一起转。',
              '温馨提示',
              {
              ...
              .then(({ value }) => {
                couponOrderExportCouponDetail({
                  ...
                  this.$router.push({
                    path: '/system_setting/task_mgr/task_mgr',
                    query: { id: orderNo }
                  })
                })
              })
