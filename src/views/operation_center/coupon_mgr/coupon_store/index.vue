    ...
    <el-form inline label-width="120px">
      ...
      <query-ext-plane :opened="opened">
        ...
        <!-- <el-form-item label="绑定日期">
                    <el-date-picker unlink-panels v-model="query.bindTime" style="width:230px;"  type="datetimerange"  value-format="yyyy-MM-dd HH:mm" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
        </el-form-item>-->
    ...
    setCouponVoid() {
      this.$confirm('是否要进行此操作', '作废提示', { type: 'warning' }).then(
        () => {
          if (
            !this.query.batchNo &&
            !this.query.couponCode &&
            !this.query.couponId
          ) {
            this.$message.warning(
              '作废电影券需先指定：电影券批次号、电影券号码、电影券ID号中的其中一种后方可执行作废操作！'
            )
          } else {
      ...
      this.$confirm('是否要进行此操作', '作废提示', { type: 'warning' }).then(
        () => {
          if (
            ...
            this.$message.warning(
              '冻结电影券需先指定：电影券批次号、电影券号码、电影券ID号中的其中一种后方可执行冻结操作！'
            )
          } else {
            const request = this.queryTemp
            request.status = 1
