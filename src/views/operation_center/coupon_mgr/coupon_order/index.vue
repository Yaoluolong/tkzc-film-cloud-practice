    ...
    <el-form inline label-width="120px">
      ...
      <el-form-item>
        ...
        <el-button
          ...
          @click="$router.push({path:'/operation_center/coupon_mgr/coupon_order/create_coupon_order'})"
        >生成订单</el-button>
      </el-form-item>
    </el-form>
    <page-table ref="table" index :query="query" :fetch="queryTable">
      ...
      <el-table-column min-width="250" label="操作" align="center" prop="oper" fixed="right">
        <template slot-scope="{row}">
          <el-button
            type="text"
            v-if="row.saleStatus !== 2"
            @click="$router.push({path:'/operation_center/coupon_mgr/sell_register',query:{orderNo:row.orderNo}})"
          >销售登记</el-button>
          <el-button
            type="text"
            @click="$router.push({path:'/operation_center/coupon_mgr/sell_detail',query:{orderNo:row.orderNo}})"
          >销售明细</el-button>
          <el-button
            type="text"
            v-if="row.isReceiveMoney=== '0' && row.isActive === '0'  && row.saleStatus !== 2"
            @click="cancelCouponOrder(row.orderNo)"
          >撤销销售</el-button>
        </template>
      </el-table-column>
    </page-table>
...
export default {
  ...
  methods: {
    cancelCouponOrder(orderNo) {
      this.$confirm(
        '撤销销售将对电影券销售订单进行撤单操作，并将销售订单中系统已生成的电影券进行作废操作，是否开始执行？',
        '撤销提示',
        { type: 'warning' }
      ).then(() => {
