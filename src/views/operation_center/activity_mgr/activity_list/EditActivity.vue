  ...
  <div class="app-container edit-price-program">
    ...
    >
      ...
      <el-form-item
        label="投放商家:"
        prop="mchList"
        v-if="!$route.query.activity_id || (ok && $route.query.activity_id)"
      >
      ...
      <el-form-item label="活动地区:" prop="area">
        <city-cascader
          ref="city"
          v-model.trim="activityParams.area"
          :rang="0"
          showAllOptions
          :clearable="true"
          placeholder="请选择"
      ...
      <el-form-item label="活动投放时间:" prop="time">
        ...
        <date-selector
          clearable
          v-if="activityParams.showtime_cycle=='weekly'"
          type="week"
          v-model="activityParams.weekTime"
          style="width:320px"
        ></date-selector>
        <el-time-picker
          v-if="activityParams.time && (activityParams.showtime_cycle=='daily' || activityParams.showtime_cycle=='weekly')"
          v-model="activityParams.limitTime"
          is-range
      ...
      <el-form-item label="上传活动图片:" prop="activity_banner">
        <pic-upload
          ...
          v-model="activityParams.activity_banner"
        ></pic-upload>
      </el-form-item>
      <el-form-item label="活动说明:" prop="description">
        <el-input
          ...
          type="textarea"
          rows="5"
          placeholder="请填写活动对象、活动规则、活动流程等活动说明"
        ></el-input>
      </el-form-item>
    ...
    >
      ...
      <el-form-item label="活动类型:" prop="activity_type">
        <el-radio-group
          v-model="settingParams.activity_type"
          size="medium"
      ...
      <div v-if="settingParams.activity_type==='cash'">
        <el-form-item label="活动可用影院" prop="cinema_group_id">
          <remote-select
            v-model="settingParams.cinema_group_id"
            clearable
            placeholder="请选择可用影院"
        ...
        <el-form-item label="活动可用影片：" prop="film_codes" v-if="settingParams.cinema_group_id">
          ...
          <film-selector
            :content-list="settingParams.filmList"
            v-if="settingParams.filmType === '1'"
            getFilmId
            v-model="settingParams.film_codes"
          ></film-selector>
        </el-form-item>
        <el-form-item label="活动可用制式：" v-if="settingParams.cinema_group_id" prop="film_copies">
          <standard-selector
            ref="film_copies"
            v-model="settingParams.film_copies"
            :cinemaGroupId="settingParams.cinema_group_id"
          ></standard-selector>
        </el-form-item>
        <el-form-item label="活动场次时间:" prop="time">
          <el-date-picker
            unlink-panels
            v-model="settingParams.time"
            type="daterange"
            value-format="yyyy-MM-dd"
            start-placeholder="开始日期"
            end-placeholder="结束日期"
          ...
          <date-selector
            clearable
            v-if="settingParams.activity_cycle=='weekly'"
            type="week"
            v-model="settingParams.weekTime"
            style="width:320px"
          ></date-selector>
          <el-time-picker
            v-if="settingParams.time && (settingParams.activity_cycle=='daily'||settingParams.activity_cycle=='weekly')"
            v-model="settingParams.limitTime"
            is-range
            value-format="HH:mm:ss"
            start-placeholder="开始时间"
            end-placeholder="结束时间"
...
export default {
  ...
  data() {
    ...
    const checkChannelList = (rule, value, cb) => {
      if (
        !this.activityParams.mchList ||
        this.activityParams.mchList.length === 0
      ) {
    ...
    const checkFilmList = (rule, value, cb) => {
      if (
        !this.settingParams.filmType ||
        (this.settingParams.filmType === '1' &&
          this.settingParams.film_codes.length === 0)
      ) {
        cb(new Error('请选择影片'))
      }
      cb()
    ...
    const checkTime = (rule, value, cb) => {
      ...
      if (
        this.activityParams.activity_cycle === 'weekly' &&
        this.activityParams.weekTime.length === 0
      ) {
    ...
    const checkSettingTime = (rule, value, cb) => {
      ...
      if (
        this.settingParams.activity_cycle === 'weekly' &&
        this.settingParams.weekTime.length === 0
      ) {
        cb(new Error('请选择活动星期'))
      }
    ...
    const checkRules = (rule, value, cb) => {
      value.forEach(res => {
        ...
        if (
          res.rule_discount_type === 'random_cash_back' &&
          (!res.rule_min_unit || !res.rule_max_unit)
        ) {
          cb(new Error('请填写完整金额'))
          return false
        }
        if (
          res.rule_discount_type === 'discount' &&
          (!res.rule_min_unit || !res.rule_max_unit)
        ) {
          cb(new Error('请填写完整金额'))
          return false
    ...
    return {
      ...
      SettingRules: {
        ...
        film_codes: {
          required: true,
          validator: checkFilmList,
          trigger: 'change'
        },
        film_copies: {
          required: true,
          validator: checkStandardList,
          trigger: 'change'
        },
  ...
  methods: {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          this.$set(
            result,
            'showtime_start_boundary',
            this.activityParams.time[0]
          )
          this.$set(
            result,
            'showtime_end_boundary',
            this.activityParams.time[1]
          )
          ...
          editActivity(result)
            .then(e => {
              ...
              this.closeTab(true)
              this.$router.push({
                path: '/operation_center/activity_mgr/activity_list'
              })
            })
            .catch(e => {
    ...
    settingApproval() {
      this.$refs.settingForm.validate(valid => {
        if (valid) {
          ...
          editActivitySettings(result)
            .then(res => {
              activitySubmitById({ activity_id: this.$route.query.activity_id })
                .then(res => {
                  loading.close()
    ...
    settingSaveParams() {
      ...
      if (this.settingParams.activity_type === 'cash') {
        result.activity_payment = this.settingParams.activity_payment
          .map(e => e)
          .join(',')
        result.activity_settlement_type = this.settingParams.activity_settlement_type
          .map(e => e)
          .join(',')
        ...
        if (
          ...
          this.settingParams.activity_days = this.settingParams.activity_days
            ? this.settingParams.activity_days + ','
            : ''
    ...
    async getInfo() {
      ...
      result.mch_id.split(',').forEach(e => {
        this.activityParams.mchList.push({ name: e, value: e, checked: true })
      })
      this.ok = true
      this.activityParams.activity_banner = result.activity_banner

      this.$set(this.activityParams, 'time', [])
      result.showtime_start_boundary
        ? this.$set(
          this.activityParams.time,
          0,
          result.showtime_start_boundary.substring(0, 10)
        )
        : ''
      result.showtime_end_boundary
        ? this.$set(
          this.activityParams.time,
          1,
          result.showtime_end_boundary.substring(0, 10)
        )
        : ''
      this.$set(this.activityParams, 'limitTime', [new Date(), new Date()])
      result.showtime_begin
        ? this.$set(this.activityParams.limitTime, 0, result.showtime_begin)
        : ''
      result.showtime_end
        ? this.$set(this.activityParams.limitTime, 1, result.showtime_end)
        : ''
      ...
      result.province_id
        ? this.$set(this.activityParams.area, 0, result.province_id)
        : ''
      result.province_id && result.region_id
        ? this.$set(this.activityParams.area, 1, result.region_id)
        : ''
      ...
      if (Object.keys(result.settings).length > 0) {
        ...
        if (this.settingParams.activity_type === 'cash') {
          ...
          this.$set(
            this.settingParams,
            'filmType',
            this.settingParams.film_codes === '-1' ? '-1' : '1'
          )
          ...
          result.settings.activity_start_boundary
            ? this.$set(
              this.settingParams.time,
              0,
              result.settings.activity_start_boundary.substring(0, 10)
            )
            : ''
          result.settings.activity_end_boundary
            ? this.$set(
              this.settingParams.time,
              1,
              result.settings.activity_end_boundary.substring(0, 10)
            )
            : ''
          this.$set(this.settingParams, 'limitTime', [])
          result.settings.activity_begin
            ? this.$set(
        ...
        this.$set(this.settingParams, 'filmList', [
          { id: '', name: '', options: [] }
        ])
      }
    }
