<template>
    <div class="app-container">
        <el-card>
            ...
            <el-form label-width="160px" label-position="right" style="width:1000px;" :model="cardInfo" :rules="rules" ref="formCard">
                ...
                <el-form-item label="电影卡号" v-if="cardInfo.style==='Z'">
                  <div v-for="item in cardInfo.codeIdDetail">
                    <span>{{item.left}}</span>
                    <span> －－ </span>
                ...
                <el-form-item label="卡内金额" prop="unitPrice" style="display:inline-block">
                  {{cardInfo.unitPrice}} 元，折合卡内点数: {{ cardInfo.unitPrice * cardInfo.scalePoint / cardInfo.scalePrice}}
                </el-form-item>
                <el-form-item label="电影卡有效期:" prop="time" >
                    {{cardInfo.startTime}} 至 {{cardInfo.endTime}}
                </el-form-item>
                <el-form-item label="销售备注:">
                  <!-- <el-input type="textarea" :row="5" v-model="cardInfo.remark" disabled></el-input> -->
                  <span>{{cardInfo.remark}}</span>
                </el-form-item>
                <el-form-item label="使用说明:">
                  <!-- <el-input type="textarea" :row="5" v-model="cardInfo.useExplain" disabled></el-input> -->
        ...
        <el-card style="margin-top:20px;">
          <!-- <el-form  label-width="140px" label-position="left" style="width:1000px;">
            <el-form-item label="审核意见:" prop="suggestion">
                <el-input v-model.trim="suggestion" type="textarea" :rows="5" placeholder="请输入审核意见" :disabled="checkStatus!=='1'"  v-show="checkStatus=='1'"></el-input>
                <span  v-show="checkStatus!=='1'">{{suggestion}}</span>
            </el-form-item>
          </el-form> -->
          <el-card shadow="never" style="width:800px;margin-top:20px;margin-bottom:15px;">
            <el-form label-width="140px" label-position="right" style="width:1000px;" v-show="checkStatus==='1'">
              <el-form-item label="审核意见:" prop="suggestion">
                <el-input v-model.trim="suggestion" type="textarea" :rows="5" placeholder="请输入审核意见" :disabled="$route.query.checkStatus!='1'"></el-input>
        ...
        <el-button style="margin-top:20px;" type="primary" @click="checkOrder" v-show="checkStatus=='1'">同意</el-button>
...
import { getInvoiceTypeList, getCardOrderInfo, cardcheckOrder } from '@/api/operationCenter'
  ...
  methods: {
    ...
    checkOrder() {
      ...
      if (this.$route.query.orderNo) {
        cardcheckOrder({ orderNo: this.$route.query.orderNo, agree: '1', suggestion: this.suggestion }).then((res) => {
    ...
    refund() {
      const _this = this
      if (this.$route.query.orderNo) {
        cardcheckOrder({ orderNo: this.$route.query.orderNo, agree: '2', suggestion: this.suggestion }).then((res) => {
  ...
  async created() {
    if (this.$route.query.orderNo) {
      ...
      getCardOrderInfo(this.$route.query.orderNo).then((res) => {
        ...
        if (res.cardInfo.checkInfo && res.cardInfo.checkInfo.suggestion !== 'null') {
