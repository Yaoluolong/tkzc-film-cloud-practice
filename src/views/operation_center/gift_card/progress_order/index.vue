    ...
    <page-table ref="table" index :query="query" :fetch="queryTable">
      ...
      >
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="isPass('2',row.orderNo,{orderNo:row.orderNo,isReceiveMoney:row.isReceiveMoney},'progress_sell_register')"
      ...
      >
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="isPass('3',row.orderNo,{orderNo:row.orderNo,invoiceType:row.invoiceType},'progress_sell_register')"
      ...
      <el-table-column min-width="110" label="激活状态" align="center" prop="isActive" fixed="right">
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="isPass('4',row.orderNo,{orderNo:row.orderNo,isActive:row.isActive},'progress_sell_register')"
...
export default {
  ...
  methods: {
    ...
    isPass(type, orderNo, query, tableName) {
      checkCardAuthority({ type: type, orderNo: orderNo })
        .then(res => {
          this.$router.push({
            path: '/operation_center/gift_card/' + tableName,
            query: query
          })
        })
        .catch(e => {
