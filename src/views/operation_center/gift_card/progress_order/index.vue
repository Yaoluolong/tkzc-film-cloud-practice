    ...
    <div class="app-container">
         ...
         <page-table ref="table" index :query="query" :fetch="queryTable">
            ...
            <el-table-column min-width="110"  label="到款状态" align="center" prop="isReceiveMoneyName" fixed="right">
              <template slot-scope="{row}">
                <el-button type="text"  @click="isPass('2',row.orderNo,{orderNo:row.orderNo,isReceiveMoney:row.isReceiveMoney},'progress_sell_register')">{{row.isReceiveMoneyName}}</el-button>
              </template>
            </el-table-column>
            <el-table-column min-width="110"  label="开票状态" align="center" prop="isDrawInvoice" fixed="right">
              <template slot-scope="{row}">
                <el-button type="text" @click="isPass('3',row.orderNo,{orderNo:row.orderNo,invoiceType:row.invoiceType},'progress_sell_register')" :disabled="row.isDrawInvoice==='无需发票'">{{row.isDrawInvoice}}</el-button>
              </template>
            </el-table-column>
            <el-table-column min-width="110"  label="激活状态" align="center" prop="isActive" fixed="right">
              <template slot-scope="{row}">
                <el-button type="text" @click="isPass('4',row.orderNo,{orderNo:row.orderNo,isActive:row.isActive},'progress_sell_register')">{{row.isActive}}</el-button>
              </template>
            </el-table-column>
            <!-- <el-table-column min-width="250"  label="操作" align="center" prop="oper">
              <template slot-scope="{row}">
                <el-button type="text" @click="$router.push({path:'/operation_center/gift_card/progress_order_submit',query:{orderNo:row.orderNo,checkStatus:row.checkStatus}})">部门初审</el-button>
                <el-button type="text" @click="$router.push({path:'/operation_center/gift_card/progress_sell_register',query:{orderNo:row.orderNo}})">财务复审</el-button>
              </template>
            </el-table-column> -->
         </page-table>
    </div>
...
import { getCardCheckPageList, checkCardAuthority } from '@/api/operationCenter'
export default {
  ...
  methods: {
    queryTable(query) {
      ...
      delete req.time
      return getCardCheckPageList(req)
    },
    refreshTable() {
      this.$refs.table.refresh()
    },
    isPass(type, orderNo, query, tableName) {
      checkCardAuthority({ type: type, orderNo: orderNo }).then(res => {
        this.$router.push({ path: '/operation_center/gift_card/' + tableName, query: query })
      }).catch(e => {
