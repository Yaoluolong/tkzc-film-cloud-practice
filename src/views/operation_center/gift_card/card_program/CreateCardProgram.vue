	...
	<div class="app-container">
		<el-form  label-width="140px" ref="form" :rules="rules" :model="params">
			...
			<el-form-item label="适用平台商家" prop="channelList" v-if="ok">
				<channel-form-selector ref="channelComponent" v-model="params.channelList"  style="width:600px;" type="1" :changeFun="channelChangeFun"></channel-form-selector>
			</el-form-item>
      ...
      start-placeholder="开始日期" end-placeholder="结束日期"  style="width:320px;"></el-date-picker>
			...
			<el-form-item label="每张电影卡--点卡可享受充值活动价:" prop="activeNum" label-width="250px" v-if="params.isOpenActive==='1'">
				<el-input v-model="params.activeNum"  style="width:100px;"></el-input>  次
				<span style="margin-left:20px;color:gray">只允许输入数值，必须大于等于0！</span>
			</el-form-item>
			<el-form-item label="充值活动时间结束后是否恢复充值原价:" prop="isRecovery" label-width="265px"  placeholder="请选择用户类型"  v-if="params.isOpenActive==='1'">
...
import { createCardRecharge, getCardRechargeInfo, updateCardRecharge } from '@/api/operationCenter'
export default{
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        channelList: [{ required: true, message: '请选择适用平台商家', trigger: 'blur' }],
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.params.channelList) {
            params.channelId = this.params.channelList.map(e => e.value).join(',')
          }
          if (this.params.time) {
          ...
          const apiTemp = this.$route.query.id ? updateCardRecharge : createCardRecharge
          apiTemp(params).then(res => {
