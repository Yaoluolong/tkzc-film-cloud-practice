<template>
  <div class="app-container">
  	<el-steps :active="step" finish-status="success" process-status="wait"  align-center style="margin-bottom:40px;">
  	  <el-step title="销售基础信息" @click.native="stepButton(1)"> 
      </el-step>
  	  <el-step title="销售发票信息" @click.native="stepButton(2)">
      </el-step>
      <el-step title="销售付款方式" @click.native="stepButton(3)">
      </el-step>
  	</el-steps>
    <!-- 销售电影基础设置 -->
    <el-card v-show="step===1"  v-if="cardInfoLoaded || !$route.query.orderNo">
      ...
      <el-form label-width="140px" label-position="left" style="width:1000px;" :model="cardInfo" :rules="cardInfoRules" ref="cardInfoForm">
	        ...
	        <el-form-item label="电影卡种类" prop="type" style="display:inline-block;width: 450px;">
	          <remote-select v-model="cardInfo.type" clearable placeholder="请选择电影卡种类" action="/systemApi/dict/getList" :query="{type:'cardType'}" style="width:230px;" @change="codeChangeFun"></remote-select>
	        </el-form-item>
	        <el-form-item label="电影卡类型" prop="style" style="display:inline-block;width: 450px;">
	          <remote-select v-model="cardInfo.style" clearable placeholder="请选择电影卡类型" action="/systemApi/dict/getList" :query="{type:'cardStyle'}" style="width:230px;" @change="codeChangeFun"></remote-select>
	        </el-form-item>
	        <el-form-item label="电影卡号" prop="codeIdDetail" v-if="cardInfo.style==='Z'">
	          <coupon-code-area :outCheckId="doGet" v-model="cardInfo.codeIdDetail" :orderNo="$route.query.orderNo" :couponType="cardInfo.type" :couponStyle="cardInfo.style"></coupon-code-area>
	        </el-form-item>
	        <el-form-item label="销售数量:" prop="num">
	            <el-input v-model.trim="cardInfo.num" placeholder="0" style="width:100px;" :disabled="cardInfo.style==='Z'||!cardInfo.style"></el-input> 张  <span style="margin-left:20px;color:red" v-show="cardInfo.style==='Z'">系统已根据您所填写的号码段自动统计出销售数量!</span>
	        </el-form-item>
	        ...
	        <div>
	          <el-form-item label="销售比例:" prop="scalePoint" style="display:inline-block">
	              <el-input type="text" v-model="cardInfo.scalePrice"  placeholder="票面金额单位元" style="width:100px;" ></el-input>
	          </el-form-item>
	          :
	          <el-form-item prop="scalePoint" label-width="0px" style="display:inline-block">
	            <el-input type="text" v-model="cardInfo.scalePoint" placeholder="卡内金额单位点" style="width:100px;" ></el-input>
	          </el-form-item>
	        ...
	        <el-form-item label="卡内金额" prop="unitPrice" style="display:inline-block">
	          <el-input type="text" v-model="cardInfo.unitPrice" placeholder="0" style="width:100px;" ></el-input>
	          元，折合卡内点数: <el-input type="text" disabled :value="cardInfo.unitPrice * cardInfo.scalePoint /cardInfo.scalePrice || '0'" placeholder="0" style="width:100px;" ></el-input>  点
	        </el-form-item>
	        <el-form-item label="电影卡有效期:" prop="time" >
	            <el-date-picker v-model="cardInfo.time" type="daterange"  value-format="yyyy-MM-dd"
	                    start-placeholder="开始日期" end-placeholder="结束日期"  style="width:320px;"></el-date-picker>
	        </el-form-item>
          <el-form-item label="销售备注:">
            <el-input type="textarea" :row="5" v-model="cardInfo.remark" placeholder="请输入该销售单的销售备注信息,便于部门主管或者财务查看"></el-input>
          </el-form-item>
          <el-form-item label="使用说明:">
            <el-input type="textarea" :row="5" v-model="cardInfo.useExplain" placeholder="请输入该销售单内的电影券在使用的时候注意事项等信息"></el-input>
          </el-form-item>
	    </el-form>
...
import { getInvoiceTypeList, getCardOrderInfo, createCardOrder, updateCardOrder } from '@/api/operationCenter'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      payInfoRules: {
        payType: { required: true, message: '请选择付款方式种类', trigger: 'blur' }
      },
      scalePointLeft: 1,
      scalePointRight: 1,
  ...
  methods: {
    ...
    stepApi(bool, step) { // 新增编辑接口
      const apiTemp = this.$route.query.orderNo ? updateCardOrder : createCardOrder
      let apiParamsTemp = {}
      switch (step) {
        case 1:
          apiParamsTemp = this.$route.query.orderNo ? { step: 1, cardInfo: this.cardInfo, orderNo: this.$route.query.orderNo } : { step: 1, cardInfo: this.cardInfo, orderNo: this.orderNo }
          break
        case 2:
          apiParamsTemp = this.$route.query.orderNo ? { step: 2, invoiceInfo: this.invoiceInfo, orderNo: this.$route.query.orderNo } : { step: 2, invoiceInfo: this.invoiceInfo, orderNo: this.orderNo }
          break
        case 3:
          apiParamsTemp = this.$route.query.orderNo ? { step: 3, payInfo: this.payInfo, orderNo: this.$route.query.orderNo } : { step: 3, payInfo: this.payInfo, orderNo: this.orderNo }
          break
      }
    ...
    submitAudit() {
      ...
      updateCardOrder({ step: 3, payInfo: this.payInfo, orderNo: orderNo, checkStatus: '1' }).then((res) => {
