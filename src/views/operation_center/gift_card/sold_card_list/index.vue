    ...
    <el-form inline label-width="120px">
      ...
      <el-form-item>
        ...
        <el-button
          ...
          @click="$router.push({path:'/operation_center/gift_card/sold_card_list/create',query:{}})"
        >销售电影卡</el-button>
      </el-form-item>
    </el-form>
      ...
      >
        <template slot-scope="{row}">
          <div v-if="row.status !== '2'">
            <el-button
              type="text"
              @click="$router.push({path:'/operation_center/gift_card/get_sale_card_list',query:{orderNo:row.orderNo}})"
            >销售单明细</el-button>
            <el-button
              type="text"
            ...
            <el-dropdown v-if="row.checkStatus == '4'">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="row.checkStatus == '4'">
                  <el-button
                    type="text"
                    @click="$router.push({path:'/operation_center/gift_card/sold_card_list/edit',query:{orderNo:row.orderNo}})"
                  >重新编辑/提交审核</el-button>
                </el-dropdown-item>
                <el-dropdown-item v-if="row.checkStatus == '4'">
...
export default {
  ...
  methods: {
    ...
    cancelSoldCardOrder(orderNo) {
      this.$confirm(
        `撤销销售将对电影卡销售订单进行撤单操作,并将销售订单中电影卡还原回未销售状态,是否开始执行?`,
        '温馨提示',
        { type: 'warning' }
      )
        .then(async() => {
