...
import { getCardTypeList } from '@/api/operationCenter'
export default {
  props: {
    value: String,
    type: {
  ...
  methods: {
    async querySearch(val, couponTypeId) {
      ...
      if (val) {
        ...
        const options = await getCardTypeList(param)
        this.couponType.options = options.filter(e => e.id)
        this.$forceUpdate()
      } else if (couponTypeId) {
        getCardTypeList({ name: '' }).then(res => {
        ...
        const param = { name: '' }
        param.type = this.type
        this.couponType.options = await getCardTypeList(param)
        this.$forceUpdate()
      }
    },
