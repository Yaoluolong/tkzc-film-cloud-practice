  ...
  <div class="app-container">
    <el-form inline label-width="100px">
      ...
      <el-form-item label="电影卡号码">
        <el-input
          v-model="query.cardCodeLeft"
          clearable
          placeholder="输入优惠卡号码查询"
          style="width:230px;"
        ></el-input>~~
        <el-input
          v-model="query.cardCodeRight"
          clearable
          placeholder="输入优惠卡号码查询"
          style="width:230px;"
        ></el-input>
      ...
      <el-form-item label="绑定会员">
        <el-input
          v-model.trim="query.bindingMobile"
          clearable
          placeholder="请输入手机号"
          style="width:240px;"
        ></el-input>
    ...
    <page-table ref="table" index :query="queryTemp" :fetch="queryTable">
      ...
      <el-table-column label="详情" align="center" prop="oper" fixed="right">
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="$router.push({path:'/operation_center/gift_card/card_detail',query:{id:row.id}})"
          >查看</el-button>
        </template>
      </el-table-column>
    </page-table>
    ...
    <el-dialog title="延期提示" :visible.sync="dialogDelaysFormVisible" width="550px">
      <el-form>
        <el-form-item label>
          <span>
            根据条件已筛选出
            <span v-text="count"></span>张电影卡，将根据填写的延期时间来重新设定卡的有效期！
          </span>
        </el-form-item>
      ...
      if (!this.messageKeyBool) {
        ...
        getSaleCardCount(this.query).then(res => {
          this.$confirm(
            '根据条件已筛选出' +
              res +
              '张电影卡，卡一旦冻结就不可使用，可以通过解冻功能来解冻，是否执行冻结',
            '作废提示',
            { type: 'warning' }
          )
            .then(() => {
