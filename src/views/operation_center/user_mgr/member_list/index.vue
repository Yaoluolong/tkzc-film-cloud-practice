    ...
    <page-table ref="table" :query="query" :fetch="queryTable">
      ...
      <el-table-column min-width="120" label="操作" align="center" prop="oper" fixed="right">
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="$router.push({path:'/operation_center/user_mgr/member_list/detail',query:{id:row.id}})"
          >查看</el-button>
          <el-button
            type="text"
            @click="$router.push({path:'/operation_center/user_mgr/member_list/edit',query:{id:row.id}})"
          >编辑</el-button>
        </template>
      </el-table-column>
    </page-table>
