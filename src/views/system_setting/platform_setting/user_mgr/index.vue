    ...
    <div class="app-container">
         <el-form inline label-width="120px">
            ...
            <el-form-item label="所属角色">
              <remote-select v-model="query.roleId" clearable  placeholder="请选择所属角色" action="/systemApi/systemAccountRole/getList" style="width:200px;"></remote-select>
            </el-form-item>
            <el-form-item >
              <el-button type="primary" icon="el-icon-search"  @click="refreshTable">查询</el-button>
              <el-button type="danger" icon="el-icon-plus"  @click="$router.push('/system_setting/platform_setting/user_mgr/create')">新增</el-button>
            </el-form-item>
         </el-form>
         <page-table ref="table" index :query="query" :fetch="queryTable">
            ...
            <el-table-column width="80"  label="是否启用" align="center" prop="status">
              <template slot-scope="{row}">
                <switch-confirm v-model="row.status" confirm-text="停用用户后用户将无法登陆平台,是否要执行此操作" :id="row.id" action="/systemApi/systemAccount/setStatus"></switch-confirm>
              </template>
            </el-table-column>
            <el-table-column min-width="160"  label="操作" align="center" prop="oper">
              <template slot-scope="{row}">
                <el-button type="text" @click="$router.push({path:'/system_setting/platform_setting/user_mgr/permission_setting',query:{id:row.id}})">功能权限</el-button>
                <el-button type="text" @click="$router.push({path:'/system_setting/platform_setting/edit_data_access',query:{id:row.id}})">数据权限</el-button>
                <el-button type="text" @click="$router.push({path:'/system_setting/platform_setting/user_mgr/edit',query:{id:row.id}})">编辑</el-button>
