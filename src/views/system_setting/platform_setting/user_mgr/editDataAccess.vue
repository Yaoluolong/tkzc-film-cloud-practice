	...
	<div class="app-container">
	  	<el-form label-width="120px"  style="width:600px;" :model="params" :rules="rules" ref="form">
	    	...
	    	<div class="elform"></div>
          	...
          	<div style="margin:4% 0;">财务中心-发行活动券报表&emsp;<el-radio v-model="params.setView" label="2">关闭</el-radio>
              <el-radio v-model="params.setView" label="1">开启</el-radio></div>
	        ...
	        <el-form-item label="" label-width="0px" v-if="params.setView==='1'" prop="couponSaleDetail">
	            <coupon-sale-detail-selector :outSearchData="params.couponOrderConditions" :dataList="couponOrderList" v-model="params.couponOrderIds" @couponOrderConditionsFunc="couponOrderConditionsFunc"></coupon-sale-detail-selector>
	        </el-form-item>
	        <div class="elform"></div>
...
export default{
  components: { customerFormSelector, ChannelAccessFormSelector, interfaceAccessFormSelector, CouponSaleDetailSelector },
  data() {
    return {
      params: {
        // 运营中心--电影券管理/电影卡管理
  ...
  methods: {
    ...
    save() {
      ...
      this.$set(this.params.couponOrderLimit, 'account', this.couponOrderLimit.account)
      this.$set(this.params.cardOrderLimit, 'account', this.cardOrderLimit.account)

      this.params.couponLimit.customer = typeof (this.couponCustomer) === 'string' ? this.couponCustomer : this.couponCustomer.join(',')
      this.params.cardLimit.customer = typeof (this.cardCustomer) === 'string' ? this.cardCustomer : this.cardCustomer.join(',')

      this.params.ticketOrder.channel = typeof (this.channelList) === 'string' ? this.channelList : this.channelList.join(',')
      this.params.ticketOrder.interface = typeof (this.interfaceTypeList) === 'string' ? this.interfaceTypeList : this.interfaceTypeList.join(',')

      this.params.couponOrderLimit.customer = typeof (this.couponOrderLimitMan) === 'string' ? this.couponOrderLimitMan : this.couponOrderLimitMan.join(',')
      this.params.cardOrderLimit.customer = typeof (this.cardOrderLimitMan) === 'string' ? this.cardOrderLimitMan : this.cardOrderLimitMan.join(',')
  ...
  created() {
    ...
    getPermissionsInfo(this.$route.query.id).then((data) => {
      ...
      this.params.couponLimit.account === '1' ? _this.$set(this.couponLimit, 'account', '1') : _this.$set(this.couponLimit, 'account', '-1')

      this.couponCustomer = this.params.couponLimit.customer

      this.params.cardLimit.account === '1' ? _this.$set(this.cardLimit, 'account', '1') : this.$set(this.params.cardLimit, 'account', '-1')
      this.cardCustomer = this.params.cardLimit.customer

      this.params.checkActivity ? '' : this.$set(this.params, 'checkActivity', '-1')

      this.channelList = this.params.ticketOrder.channel
      this.interfaceTypeList = this.params.ticketOrder.interface

      this.params.couponOrderLimit.account === '1' ? this.$set(this.couponOrderLimit, 'account', '1') : this.$set(this.couponOrderLimit, 'account', '-1')

      this.couponOrderLimitMan = this.params.couponOrderLimit.customer

      this.params.cardOrderLimit.account === '1' ? this.$set(this.cardOrderLimit, 'account', '1') : this.$set(this.cardOrderLimit, 'account', '-1')
      this.cardOrderLimitMan = this.params.cardOrderLimit.customer

      this.params.couponOrderConditions = this.params.couponOrderConditions ? this.params.couponOrderConditions : { orderNo: '', customerName: '' }
      this.ok = true
    })
  }
}
