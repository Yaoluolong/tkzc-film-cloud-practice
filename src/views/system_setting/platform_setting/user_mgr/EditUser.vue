  ...
  <div class="app-container">
    <el-form label-width="120px"  style="width:600px;" :model="params" :rules="rules" ref="form">
      ...
      <el-form-item label="所属角色" prop="roleId">
        <remote-select v-model="params.roleId"  placeholder="请选择所属角色" action="/systemApi/systemAccountRole/getList" style="width:100%"></remote-select>
      ...
      <el-form-item label="用户类型" prop="type" placeholder="请选择用户类型" >
        <tip content="用户账户归属，用于登录时的依据可登录哪个PC后台！">
          <el-select v-model="params.type" style="width:100%">
              ...
              <el-option value="2" label="商户端用户"></el-option>
            </el-select>
        </tip>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        userName: { required: true, max: 10, message: '请输入用户名,最多10个英文或数字字符', trigger: 'blur' },
        realName: { required: true, max: 10, message: '请输入用户姓名,最多10个字', trigger: 'blur' },
  ...
  methods: {
    save() {
      this.$refs.form.validate(async(valid) => {
        if (valid) {
          await (this.params.id ? updateUser(this.params) : createUser(this.params))
