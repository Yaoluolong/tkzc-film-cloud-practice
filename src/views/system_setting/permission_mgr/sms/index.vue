<template>
    <div class="app-container">
        ...
        <el-form inline label-width="120px">
     	...
     	<!-- 平台 -->
     	<page-table ref="table" index :query="query" :fetch="queryTable" v-show="query.smsStatus ==='0'">
            <el-table-column min-width="100"  label="短信平台" align="center" prop="typeName" ></el-table-column>
            <el-table-column min-width="120"  label="账户" align="center" prop="accountName" show-overflow-tooltip></el-table-column>
        ...
        <!-- 模板 -->
        <page-table ref="moduleTable" index :query="modulesQuery" :fetch="modulesQueryTable" v-show="query.smsStatus ==='1'">
            <el-table-column min-width="80"  label="模板类型" align="center" prop="parentTypeName" ></el-table-column>
            <el-table-column min-width="140"  label="模板名称" align="center" prop="typeStrName" show-overflow-tooltip></el-table-column>
        ...
        <el-dialog :title=diaLogTitle :visible.sync="dialogFormVisible" width="40%">
            <el-form label-width="120px" :model="params" :rules="rules" ref="form">
              ...
              <el-form-item label="短信签名" prop="sign">
                <el-input v-model="params.sign" label="描述文字" style="width:200px"></el-input>
              </el-form-item>
            </el-form>
        ...
        <el-dialog :title=diaLogModulesTitle :visible.sync="dialogModulesVisible" width="50%">
            <el-form label-width="120px" :model="module" :rules="moduleRules" ref="modulesForm">
              <el-form-item label="模板类型" prop="parentType">
                <remote-select v-model="module.parentType" clearable placeholder="请选择" action="/systemApi/dict/getList" :query="{type:'smsTemplateType'}" @change="parentTypeChange"></remote-select>
              </el-form-item>
              <el-form-item label="模板名称" prop="typeStr">
                <el-select v-model="module.typeStr" clearable placeholder="请选择模板名称">
                	<el-option
              ...
              <el-form-item label="短信平台" prop="smsType">
                <remote-select v-model="module.smsType" clearable placeholder="请选择" action="/systemApi/dict/getList" :query="{type:'smsType'}"></remote-select>
              </el-form-item>
              ...
              <el-form-item :show-message="false" label="短信模版:" style="margin-top:1%" prop="content" v-if="module.parentType && module.typeStr">
                <el-input v-model.trim="module.content" :id="'content_'+ module.typeStr" style="width:90%;" placeholder="请输入内容"></el-input>  <br>
...
<script>
import { getSmsPageList, getSmsInfo, getSmsTemplatePageList, getTemplateParamList, getTemplateNameList, getSmsTemplateInfo, createSms, updateSms, deleteSms, createSmsTemplate, updateSmsTemplate, deleteSmsTemplate } from '@/api/systemSetting'
...
export default{
  ...
  data() {
    return {
      ...
      rules: {
        ...
        accountName: { required: true, message: '请输入平台账号', trigger: 'change' },
        password: { required: true, message: '请输入平台密码', trigger: 'change' },
        sign: { required: true, message: '请输入短信签名', trigger: 'change' }
      },
      moduleRules: {
        parentType: { required: true, message: '请选择模板类型', trigger: 'change' },
        typeStr: { required: true, message: '请选择模板名称', trigger: 'change' },
        smsType: { required: true, message: '请选择短信平台', trigger: 'change' },
        templateCode: { required: true, message: '请输入阿里code', trigger: 'change' },
      ...
      modulesList: {
        ...
        dx1_0: {
          ...
          shortcuts: [
            '日期', '影院名称', '影片名称', '订单号', '取票码', '验证码', '取票1序列号'
          ]
        },
        vista1_0: {
          parentType: '2',
        ...
        exportTask: {
          ...
          templateCode: '',
          content: '',
          format: '样式：尊敬的【平台名称】客户，本次导出的电影券/卡文件【文件名称】请使用解压码【解压密码】来打开文件，请注意保管解压密码，如有疑问请咨询：4008-616-123',
          shortcuts: [
            '平台名称', '文件名称', '解压密码'
          ]
        },
        ticketFailAuto: {
          ...
          templateCode: '',
          content: '',
          format: '样式：尊敬的【平台名称】会员：您购买在【影院名称】的【放映时间】放映【影片名称】的影票，订单未能成功系统已自动为您操作退款，支付金额会在1-7个工作日返回支付来源。如想继续观影您可重新下单购买，感谢您的理解与支持，如有疑问请致电客服热线400-8507-010，祝您生活愉快！',
          shortcuts: [
            '平台名称', '影院名称', '影片名称', '放映时间'
          ],
          show: true
        ...
        platformManual: {
          ...
          templateCode: '',
          content: '',
          format: '样式：尊敬的【平台名称】会员：您购买在【影院名称】的【放映时间】放映【影片名称】的影票，已由人工客服介入为您操作退款退票（影院设备故障或来电申请），支付金额会在1-7个工作日返回支付来源。如想继续观影您可重新下单购买，感谢您的理解与支持，如有疑问请致电客服热线400-8507-010，祝您生活愉快！',
          shortcuts: [
            '平台名称', '影院名称', '影片名称', '放映时间'
          ],
          show: true
        ...
        marketCouponWarn: {
          ...
          templateCode: '',
          content: '',
          format: '样式：您好：【业务人员】销售给【客户名称】的业务预付金已超限，请及时督促处理，请在【天数】内完成对账。',
          shortcuts: [
            '业务人员', '客户名称', '销售订单', '天数'
          ],
          show: true
        ...
        plateformOrderWarn: {
          ...
          templateCode: '',
          content: '',
          format: '样式：您好：【平台商家】缴纳的【预付金】使用已达到【额度】，请及时督促处理，请在【天数】内完成对账。',
          shortcuts: [
            '平台商家', '预付金', '额度', '天数'
          ],
          show: true
        ...
        couponApprovalSms: {
          ...
          templateCode: '',
          content: '',
          format: '样式：电影券销售单审批：您有一笔【业务人员】的电影券销售单需要您进行【审批环节】，请及时处理，销售单号为：【销售订单号】',
          shortcuts: [
            '业务人员', '审批环节', '销售订单号'
          ],
          show: true
        ...
        cardApprovalSms: {
          ...
          templateCode: '',
          content: '',
          format: '样式：电影卡销售单审批：您有一笔【业务人员】的电影卡销售单需要您进行【审批环节】，请及时处理，销售单号为：【销售订单号】。',
          shortcuts: [
            '业务人员', '审批环节', '销售订单号'
          ],
          show: true
        ...
        activeApprovalSms: {
          ...
          templateCode: '',
          content: '',
          format: '样式：营销活动审批：您有一个【活动类型】的营销活动需要您进行【审批环节】操作，请及时处理，创建人是【创建人】，活动序号为【活动序号】。',
          shortcuts: [
            '活动类型', '审批环节'
          ],
          show: true
        ...
        bindMobile: {
          ...
          templateCode: '',
          content: '',
          format: '样式：您正在进行会员关联手机号的操作，本次的验证码是：【验证码】，验证码在10分钟之内有效，超时请重新获取，请不要泄露给他人。',
          shortcuts: [
            '验证码'
          ],
          show: true
        ...
        changeBindingMobile: {
          ...
          templateCode: '',
          content: '',
          format: '样式：您正在进行会员关联手机号更换的操作，本次的验证码是：【验证码】，验证码在10分钟之内有效，超时请重新获取，请不要泄露给他人。',
          shortcuts: [
            '验证码'
          ],
          show: true
        ...
        oneValue: {
          ...
          templateCode: '',
          content: '',
          format: '样式：尊敬的【平台商家】会员：您已成功订购【放映时间】在【影院名称】的【影片名称】电影票，请凭取票信息【变量1】至影城柜台或自助机取票，祝您观影愉快！',
          shortcuts: [
            ''
          ],
          show: true
        ...
        twoValue: {
          ...
          templateCode: '',
          content: '',
          format: '样式：尊敬的【平台商家】会员：您已成功订购【放映时间】在【影院名称】的【影片名称】电影票，请凭取票信息【变量1】及【变量2】至影城柜台或自助机取票，祝您观影愉快！',
          shortcuts: [
            ''
          ],
          show: true
        ...
        threeValue: {
          ...
          templateCode: '',
          content: '',
          format: '样式：尊敬的【平台商家】会员：您已成功订购【放映时间】在【影院名称】的【影片名称】电影票，请凭取票信息【变量1】及【变量2】和【变量3】至影城柜台或自助机取票，祝您观影愉快！',
          shortcuts: [
            ''
          ],
          show: true
        ...
        activityWarn: {
          ...
          templateCode: '',
          content: '',
          format: '样式：您好：【活动名称】的【活动款】使用已达到【额度】，请及时处理。',
          shortcuts: [
            ''
          ],
          show: true
  ...
  methods: {
    ...
    async cancel(id) {
      this.$confirm(`删除短信平台后，将导致平台内有调用到此短信平台的接口全部失效，确认是否删除！`, '温馨提示', { type: 'warning' }).then(async() => {
        await deleteSms(id)
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async cancelTemplate(id) {
      this.$confirm(`删除短信模版后，如果系统内有调用此短信模版的地方，将导致用户无法接收到短信信息提醒，是否确认删除！`, '温馨提示', { type: 'warning' }).then(async() => {
