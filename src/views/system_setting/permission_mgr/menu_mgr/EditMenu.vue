     ...
     <el-dialog :title="title" :visible.sync="dialogVisible" width="600px" @close="resetFields('form')">
        <el-form label-width="120px"  :model="params" :rules="rules" ref="form">
            <el-form-item label="菜单名称" prop="menuName">
              <el-input  v-model.trim="params.menuName" placeholder="请输入菜单名称,最多9个字"></el-input>
          </el-form-item>
          <el-form-item  label="菜单地址" prop="menuUrl">
              <el-input  v-model.trim="params.menuUrl" placeholder="请输入菜单访问地址"></el-input>
          </el-form-item>
          <el-form-item  label="菜单同级序号" prop="sort">
              <el-input-number  v-model.trim="params.sort" ></el-input-number>
          </el-form-item>
    ...
    export default {
      ...
      data() {
        return {
          ...
          level: '',
          rules: {
            menuName: { required: true, max: 9, message: '请输入菜单名称,最多9个字', trigger: 'blur' },
            menuUrl: { required: true, message: '请输入菜单访问地址', trigger: 'blur' },
      ...
      methods: {
        ...
        save() {
          this.$refs.form.validate(async(valid) => {
            if (valid) {
              await (this.params.id ? updateMenu(this.params) : createMenu(this.params))
