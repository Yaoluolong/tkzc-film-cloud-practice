...
import { saveActivityPayConfig } from '@/api/activitySetting'
export default {
  name: 'activityPay',
  props: {
    value: Object,
  ...
  methods: {
    submitForm() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          saveActivityPayConfig(this.payParam).then(() => {
            this.$message({
              message: '保存成功',
              type: 'success'
            })
            this.closeTab(true)
            // this.$router.push('/system_setting/activity_pay_setting/activity_pay')
          })
        } else {
          return false
        }
