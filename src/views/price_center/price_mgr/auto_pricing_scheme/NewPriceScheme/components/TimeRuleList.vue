  ...
  <div>
    <template v-for="(item, index) in timeRule">
      <component
        ...
        ref="timeRule"
        class="time-rule-item mt10"
      ></component>
    </template>
  </div>
</template>
<script>
import DateRnageRuleItem from './TimeRuleItemDateRange'
import WeeksRuleItem from './TimeRuleItemWeeks'
import DateRuleItem from './TimeRuleItemDate'
import {
  createTimeRuleItem,
  DATE_TYPES_MAP
...
export default {
  ...
  computed: {
    componentName() {
      if (+this.timeType === 3) return 'DateRuleItem'
