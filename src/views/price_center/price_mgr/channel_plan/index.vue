    ...
    <el-form inline label-width="120px">
      ...
      <el-form-item>
        ...
        <el-button
          ...
          @click="updatePlan(query.channelId,query.cinemaId,query.cinemaName)"
        >更新排期及价格</el-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    goToManage(row) {
      ...
      this.$router.push({
        path: '/price_center/price_mgr/channel_plan/channel_management'
      })
    },
    async updatePlan(channelId, cinemaId, cinemaName) {
      if (!cinemaId) {
        this.$confirm(
          '在未选择渠道商或影院的情况下默认更新所有渠道商的影院排期，此时更新时间会比较长，建议选择单家渠道商进行更新！',
          '温馨提示',
          {
