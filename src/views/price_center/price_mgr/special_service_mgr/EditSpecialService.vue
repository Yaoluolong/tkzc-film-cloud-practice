<template>
  <div class="app-container edit-price-program">
    <el-form label-width="140px" label-position="left" style="width:1110px;" :model="params" :rules="rules" ref="form">     
      ...
      <div v-if="params.way && rule" v-for="(rule,index) in params.ruleList" :key="rule.key">
          <el-form-item label="选择影厅：" prop="hallId" v-if="params.way === '1'">
             <hall-selector :index="index" :selectTypeNotAllCallBack="selectNotAllHallCallBack" :selectTypeAllCallBack="selectAllHallCallBack" ref="hallSelector" :rule="rule" v-model="rule.hallId" :disableHallIds="getDisablehallIds(index)" :cinemaList="cinemaList"></hall-selector>
          </el-form-item>
          <el-form-item label="选择制式：" prop="standard" v-if="params.way === '2'">
                <standard-selector ref="standard" :selectTypeNotAllCallBack="selectNotAllHallCallBack" :selectTypeAllCallBack="selectAllHallCallBack" v-model="rule.standard" :disableStandards="getDisableStandards(index)" :cinemaList="cinemaList"></standard-selector>
          </el-form-item>
          <el-form-item label="选择影片：" prop="filmId" v-if="params.way === '3'">
            ...
            <film-selector :content-list="rule.contentList" v-if="rule.filmType === '1'" getFilmId v-model="rule.filmId"></film-selector>
          </el-form-item>
          <el-form-item label="针对支付方式：" prop="payType">
              <el-checkbox v-model="rule.selectAllPayTypeFlag" @change="ddd(rule)" style="float:left" label="-1">全选</el-checkbox>        
              <el-checkbox-group style="margin-left:70px" v-model="rule.selectedPayTypeArray" >   
                <el-checkbox v-for=" payType in allPayTypeArray" :key="payType.payCode" :label="payType.payCode">{{payType.payName}}</el-checkbox> 
              </el-checkbox-group>  
          </el-form-item>
          <el-form-item label="增设特殊服务费："  prop="servicePrice">
              <el-input style="width:150px;" v-model.trim="rule.servicePrice" placeholder="0.00"><template slot="append">元</template></el-input>  
              <div class="oper" style="float:right">
                <i class="el-icon-remove info" title="删除" @click="delRule(index)"  v-if="params.ruleList.length!=1"></i>
                <i class="el-icon-circle-plus danger" title="添加" @click="addNewRule(params.way)" v-if="index==params.ruleList.length-1 && rule.showAdd"></i>
              </div>
          </el-form-item>
...
import { createSpecialServicePrice, getChannelSpecialServicePriceInfo, updateSpecialServicePrice } from '@/api/priceCenter'
// getPayTypeList,
export default {
  name: 'edit_special_service_mgr',
  components: { CinemaSelector, ChannelSelector, HallSelector, StandardSelector, FilmSelector, ChannelIdSelector, DateSelector },
  data() {
    return {
      channelId: '',
      cinemaList: [],
      ...
      rules: {
        ...
        dateType: { required: true, message: '请选择执行时间', trigger: 'blur' },
  ...
  watch: {
    ...
    'params.way': { // 改变增设方式时将ruleList清空,并根据增设类型增加元素
      handler: function(newValue, oldValue) {
        // 编辑时, 第一次选择不新增规则
  ...
  methods: {
    ...
    checkCinemaList() { // 选择增设方式前先要判断是否选择了影院
    ...
    save() {
      ...
      result.ruleList = this.params.ruleList.map((e) => {
        ...
        if (this.params.way === '1') { // 根据影厅
          temp.hallId = e.hallId
        } else if (this.params.way === '2') { // 根据制式
          temp.standard = e.standard
        } else if (this.params.way === '3') { // 根据影片
          if (e.filmType === '-1') {
            temp.filmId = '-1'
          } else {
  ...
  async created() {
    ...
    if (this.$route.query.id) {
      const result = await getChannelSpecialServicePriceInfo(this.$route.query.id)
      ...
      const ruleTemp = result.ruleList.map((e) => {
        ...
        if (result.way === '1') { // 根据影厅
          temp.hallId = e.hallId
        } else if (result.way === '2') { // 根据制式
          temp.standard = e.standard
        } else if (result.way === '3') { // 根据影片
          temp.filmId = e.filmId
