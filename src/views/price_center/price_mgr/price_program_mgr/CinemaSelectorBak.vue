<template>
  <div>
    ...
    <div class="tag-plane" v-if="selectedCinemaList&&selectedCinemaList.length>0&&selectTypeBak !== '1'" style="max-height:500px;overflow-y: auto;">
      <el-tag type="danger" @close="delFun(index,cinema)" class="selected-item" v-for="(cinema,index) in selectedCinemaList" closable :key="index">{{cinema.name}}</el-tag>
    </div>
    <!-- 选择所有影院时显示 -->
    ...
    <el-dialog title="选择影院"  :visible.sync="dialogVisible" width="600px" @close="resetFields('form')">
        ...
        <div class="checkbox-plane" v-show="selectType === '2'">
          ...
          <div class="content">
            <div class="check-item" v-for="cinema in cinemaList" :key="cinema.value+new Date().getTime()">             
