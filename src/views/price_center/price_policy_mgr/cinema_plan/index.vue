<template>
    <div class="app-container">
         <el-form inline label-width="120px">
           <el-form-item label="票务系统商">
            ...
            <el-form-item label="排期编码">
              <el-input v-model="query.featureAppNo" clearable placeholder="请输入影片排期编码" style="width:200px;"></el-input>
            </el-form-item>            
            <el-form-item label="放映日期">
              <el-date-picker v-model="query.time" type="datetimerange"  value-format="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm" :default-time="['00:00:00', '23:59:59']" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
            </el-form-item>
         ...
         <page-table ref="table" index :query="query" :fetch="queryTable">
            ...
            <el-table-column   label="影片票价" align="center" prop="ticketPrice" :formatter="priceFormatter"></el-table-column>
         ...
         <el-dialog v-if="syncErrCinema && syncErrCinema.length > 0" :show-close="false" center :visible="syncErrVisible" width="500px">
            <div style="border-bottom:1px solid #CCCCCC;line-height:25px" slot="title">
                  排期同步结果!
            </div>
            <div style="max-height:500px;overflow-y: auto;">
              <div  v-for="(el,index) in syncErrCinema" :key="index" style="max-height:600px;text-align:left;font-size:14px;" slot="title">
                  <div >{{el}}</div>
              </div>
            </div>
            <div slot="footer">
...
import { getCinemaPlanPageList, syncCinemaPlan, getSyncCinemaPlanPercent } from '@/api/mallCenter'
...
export default {
  ...
  methods: {
    ...
    async doUpdatePlan() {
      ...
      try {
        const batchNo = await syncCinemaPlan({ cinemaName: this.query.cinemaName, interfaceTypeId: this.query.interfaceTypeId })
    ...
    async updatePlan() {
      if (this.query.cinemaName || this.query.interfaceTypeId) {
        if (!this.query.cinemaName || !this.query.interfaceTypeId) {
          this.$message.info('系统将更新全部影院排期')
        }
        this.doUpdatePlan()
      } else {
        this.$confirm(`在未选择影院的情况下默认更新所有已启用的影院的排期,此时更新时间会较长,建议选择当家影院进行更新!`, '温馨提示',
          {
