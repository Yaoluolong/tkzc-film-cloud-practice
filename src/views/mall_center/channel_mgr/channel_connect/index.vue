  ...
  <div class="app-container">
    <el-form inline label-width="120px">
      <el-form-item label="商家名称">
        <!-- <el-input clearable v-model="query.name" clearable placeholder="请输入渠道商名称查询" style="width:200px;"></el-input> -->
        <channel-id-selector v-model="query.channelId" style="width:200px;"></channel-id-selector>
      </el-form-item>
      <el-form-item label="商家类型">
        <el-select clearable v-model="query.type">
      ...
      <el-form-item>
        ...
        <el-button
          type="danger"
          icon="el-icon-plus"
          @click="$router.push('/mall_center/channel_mgr/channel_connect/create')"
        >新增</el-button>
      </el-form-item>
    </el-form>
    <page-table ref="table" index :query="query" :fetch="queryTable">
      ...
      <el-table-column label="是否启用" align="center" prop="status">
        <template slot-scope="{row}">
          <switch-confirm
            v-model="row.status"
            activeValue="1"
            confirmText="停用商家将导致涉及影院的常规价格方案自动下架以及线上购票链接或接口无法正常访问，但相关交易数据仍将保留。是否要执行此操作？"
            :id="row.id"
            action="/systemApi/channel/setStatus"
          ></switch-confirm>
        </template>
      </el-table-column>
      <el-table-column min-width="250" label="操作" align="center" prop="oper" fixed="right">
        <template slot-scope="{row}">
          <el-button
            type="text"
            @click="$router.push({path:'/mall_center/channel_mgr/view_channel',query:{id:row.id}})"
          >查看</el-button>
    ...
    <el-dialog :visible.sync="centerDialogVisible" width="500px" :show-close="false" center>
      <div slot="title">商家C端访问链接生成成功</div>
...
export default {
  ...
  methods: {
    ...
    editChannel(row) {
      ...
      this.$router.push({
        path: '/mall_center/channel_mgr/channel_connect/edit',
        query: { id: row.id }
      })
      // }
    },
    ...
    deleteChannel(row) {
      this.$confirm(
        `删除商家将导致涉及影院的常规价格方案自动下架以及线上购票链接或接口无法正常访问，但相关交易数据仍将保留。是否要执行此操作？`,
        '删除提示',
        { type: 'warning' }
      ).then(async() => {
