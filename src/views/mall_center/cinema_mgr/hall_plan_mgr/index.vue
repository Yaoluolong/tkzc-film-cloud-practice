...
import { getCinemaAndPlanInfo, syncCinemaPlan, getSyncCinemaPlanPercent, getSyncHallPercent, syncHall } from '@/api/mallCenter'
...
export default {
  ...
  methods: {
    ...
    async doUpdatePlan() {
      ...
      try {
        const batchNo = await syncCinemaPlan({ cinemaName: this.query.cinemaName, interfaceTypeId: this.query.interfaceId })
    ...
    async updateSeat() {
      if (this.query.cinemaName) {
        ...
        const _this = this
        this.$confirm(`在未选择影院的情况下默认更新所有已启用的影院影厅以及对于影厅的座位图,此时更新时间会较长,建议选择单家影院进行更新!`, '温馨提示',
          {
            confirmButtonText: '继续更新',
            cancelButtonText: '选择单家',
            type: 'warning'
