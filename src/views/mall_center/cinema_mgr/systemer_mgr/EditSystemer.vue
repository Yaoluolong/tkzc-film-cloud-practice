  ...
  <div class="app-container">
    ...
    >
      <el-form-item label="系统商名称" prop="name">
        <el-input clearable v-model.trim="params.name" placeholder="请输入渠道商名称,最多20个字"></el-input>
      </el-form-item>
      ...
      <el-form-item label="选择版本类" prop="class">
        <remote-select
          ...
          clearable
          placeholder="请选择版本类"
          action="/systemApi/interfaceType/getInterfaceClassList"
          style="width:200px;"
        ></remote-select>
      </el-form-item>
      <el-form-item label="选择系统商接入类型：">
        <el-radio-group v-model="params.dockType" prop="dockType">
          <el-radio
            v-for="(item,index) in DOCK_TYPES"
            :key="index"
            :label="item.value"
          >{{item.label}}</el-radio>
        </el-radio-group>
      ...
      <el-form-item label="请求系统商地址" prop="apiUrl">
        <el-input clearable v-model.trim="params.apiUrl" placeholder="请输入正确访问系统商地址"></el-input>
      </el-form-item>
      <el-form-item label="请求账号" prop="apiAccount">
        <el-input clearable v-model.trim="params.apiAccount" placeholder="请输入系统商分发账号"></el-input>
      </el-form-item>
      <el-form-item label="请求密码" prop="apiPassword">
        <el-input clearable v-model.trim="params.apiPassword" placeholder="请输入系统商分发密码"></el-input>
      </el-form-item>
      <el-form-item label="请求系统商参数" prop="otherInfo">
        ...
        <arg-edit-plane-two
          ...
          help="新增每次请求系统商时的键值，例如：ticketUrl，callBackUrl。"
        ></arg-edit-plane-two>
      </el-form-item>
      <el-form-item label="关联平台服务器" prop="requestUrl">
        <el-input clearable v-model.trim="params.requestUrl" placeholder="输入关联平台服务器"></el-input>
      </el-form-item>
      <el-form-item label="请求平台账号" prop="planAccount">
        <el-input clearable v-model.trim="params.planAccount" placeholder="输入请求平台账号"></el-input>
      </el-form-item>
      <el-form-item label="请求平台密码" prop="password">
        <el-input clearable v-model.trim="params.password" placeholder="输入请求平台密码"></el-input>
      </el-form-item>
      <el-form-item label="请求超时" prop="requestOverTime">
        <tip content="每次请求系统商的超时时间设定，结合系统商接口文档来设定合理超时时间。" style="width:50%">
          <el-input clearable v-model.trim="params.requestOverTime" placeholder="请输入请求超时">
      ...
      <el-form-item label="响应超时" prop="responseOverTime">
        <tip content="每次请求系统商后数据返回的超时时间设定，结合系统商接口文档来设定合理超时时间。" style="width:50%">
          <el-input clearable v-model.trim="params.responseOverTime" placeholder="请输入响应超时">
            <template slot="append">秒</template>
          </el-input>
        </tip>
      </el-form-item>
      ...
      <el-form-item label="订单状态轮询时间" prop="orderRequestOverTime">
        <tip content="根据系统商要求填写订单支付成功后请求出票的过程时间，直到获取到返回订单成功的状态为止。" style="width:50%">
          <el-input clearable v-model.trim="params.orderRequestOverTime" placeholder="请输入轮询时间">
      ...
      <el-form-item label="订单状态轮询间隔" prop="sleepTime">
        <tip content="订单支付成功后请求出票的过程，间隔多长时间请求获取订单的状态。" style="width:50%">
          <el-input clearable v-model.trim="params.sleepTime" placeholder="请输入轮询间隔">
            <template slot="append">秒</template>
          </el-input>
        </tip>
      </el-form-item>
      <el-form-item label="订单锁座时间" prop="unLockSeatTime">
        <tip content="订单锁座时间也就是订单支付的有效时长，超出配置的时间也就是订单超时。" style="width:50%">
          <el-input clearable v-model.trim="params.unLockSeatTime" placeholder="请输入锁座时间">
            <template slot="append">分钟</template>
          </el-input>
        </tip>
      </el-form-item>
      ...
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          if (this.$route.query.id) {
            ...
            message =
              '确定新增该票务系统商吗？同意新增票务系统商成功后系统将自动执行读取影院的操作！'
          }
          this.$confirm(message, '温馨提示', { type: 'warning' }).then(
            async() => {
