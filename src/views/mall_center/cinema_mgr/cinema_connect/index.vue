  ...
  <div class="app-container">
    ...
    <el-form inline label-width="100px">
      ...
      <!-- <el-form-item label="广电编码">
              <el-input clearable v-model="query.cinemaId" clearable placeholder="请输入广电编码" style="width:200px;"></el-input>
      </el-form-item>-->
      <el-form-item>
        ...
        <el-button
          type="danger"
          icon="el-icon-plus"
          v-if="cinemaType==='normal'"
          @click="$router.push('/mall_center/cinema_mgr/cinema_connect/create')"
        >新增</el-button>
    ...
    <page-table
      ...
      :query="query"
      :fetch="queryTable"
      @selection-change="changeSelectedFun"
    >
      <el-table-column
        width="50"
        label="平台影院ID"
        v-show="cinemaType !== 'abnormal'"
        align="center"
        prop="id"
        type="selection"
      ></el-table-column>
      ...
      <el-table-column
        ...
        show-overflow-tooltip
      >
        <template slot-scope="{row}">{{row.provinceName}}/{{row.cityName}}/{{row.countyName}}</template>
      </el-table-column>
      ...
      <el-table-column min-width="180" label="操作" align="center" prop="oper" fixed="right">
        <template slot-scope="{row}">
          <el-button
            type="text"
            v-if="cinemaType === 'normal'"
            @click="$router.push({path:'/mall_center/cinema_mgr/view_cinema',query:{id:row.id}})"
          >查看</el-button>
          <el-button
            type="text"
            @click="$router.push({path:'/mall_center/cinema_mgr/cinema_connect/edit',query:{id:row.id}})"
          >编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    deleteSystemer(row) {
      this.$confirm(
        `删除影院后将导致相关的渠道商无该影院的后果，但不会删除该影院所产生的交易数据。是否要执行此操作？`,
        '删除提示',
        { type: 'warning' }
      ).then(async() => {
