  ...
  <div>
    <template v-if="!showAddNewCustom">
      <el-select
        ...
        placeholder="输入客户名称模糊查询"
        :style="selectStyle"
        :loading="loading"
      >
        <el-option
          v-for="item in custom.options"
          :key="item.value"
          :label="item.name"
          :value="item.value"
        ></el-option>
      ...
      <el-button
        type="text"
        v-if="isAddNew"
        @click="showAddNewCustom = true;addParam.name = ''"
      >+添加平台新客户</el-button>
    </template>
    <template v-if="showAddNewCustom">
      <el-input clearable placeholder="请输入客户名称" v-model="addParam.name" style="width:250px"></el-input>
...
export default {
  ...
  methods: {
    async querySearch(val, customId) {
      if (val) {
        const options = await getCustomerList({
          name: val,
