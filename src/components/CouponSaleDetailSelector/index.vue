  ...
  <div class="cinemaContent">
    ...
    <div class="cinemaContentainer">
      <el-form inline :model="query" ref="form" class="cinemaCont">
        ...
        <el-form-item label="客户">
          <remote-select-multiple
            ...
            showAllOption
            placeholder="选择客户名称"
            action="/systemApi/customer/getList"
          ></remote-select-multiple>
        </el-form-item>
      ...
      <el-radio-group v-model="selectType" style="margin-top:20px" @change="selectTypeChange">
        <el-radio :label="'1'">所有订单</el-radio>
        <el-radio :label="'2'">指定订单</el-radio>
      </el-radio-group>
...
export default {
  ...
  watch: {
    multipleSelection(val) {
      val.indexOf('-1') > -1
        ? this.$emit('input', '-1')
        : this.$emit('input', val.map(e => e.orderNo).join(','))
    }
  },
  methods: {
  ...
  created() {
    this.query.orderNo = this.outSearchData.orderNo
      ? this.outSearchData.orderNo
      : ''
    this.query.customerName = this.outSearchData.customerName
      ? this.outSearchData.customerName
      : ''
