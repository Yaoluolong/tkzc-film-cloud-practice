        ...
        <div>
          <el-form>
            ...
            <el-form-item label="模板类型：" prop="smsTemplateType" style="margin-bottom:20px;">
              <remote-select v-model="nodeList.smsTemplateType" clearable placeholder="请选择" action="/systemApi/dict/getList" :query="{type:'smsTemplateType'}" @change="parentTypeChange"></remote-select>
            </el-form-item>
            ...
            <el-form-item label="通知接收人：" style="margin-bottom:20px;" class="mobileStyle">
              <el-input v-model="nodeList.mobile" type="textarea" :rows="5" style="width:600px;" placeholder="接收号码与接收号码之间用英文“,”隔开"></el-input>
            </el-form-item>
          </el-form>
        </div>
    </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      nodeList: {
        handler(val) {
          const accountId = this.selectedUserList.map(e => {
            return e.value
          }).join(',')
          const nodeResult = {
            process: this.processId,
    ...
    methods: {
      ...
      doChangeFun(val) {
        ...
        const nodeResult = {
          process: this.processId,
          priority: '1',
          accountId: this.selectType === '1' ? '-1' : this.selectedUserList.map(e => { return e.value }).join(','),
          accountList: this.selectType === '1' ? mergeArray(this.selectedUserList, this.UserList.list) : this.UserList.list.filter(e => e.checked),
          messageType: this.nodeList.messageType,
          smsTemplateType: this.nodeList.smsTemplateType,
          smsTemplateStr: this.nodeList.smsTemplateStr,
          mobile: this.nodeList.mobile
