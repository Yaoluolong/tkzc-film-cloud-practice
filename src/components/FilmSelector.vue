    ...
    <div class="film-selector-box">
      <div v-for="(film,index) in films" :key="index" class="film-select-plane">
        ...
        <div class="oper">
          ...
          <i class="el-icon-circle-plus danger" title="添加" @click="newFilm(film)" v-if="index==films.length-1"></i>
        </div>
      </div>
    </div>
</template>
  ...
  methods: {
    async querySearchAsyncTwo(film) {
      if (!this.searchVal) {
        ...
        film.options = await getFilmIdList({})
      }
    },
    ...
    async newFilm() {
      // this.films.push({ id: '', name: '', options: [] })
      this.films.push({ id: '', name: '', options: await getFilmIdList({}) })
    },
    removeFilm(index) {
  ...
  created() {
    ...
    if (this.contentList) {
      this.films = this.contentList.map(e => {
        return { id: e.value, name: e.name, options: [e] }
      })
    }
  }
}
