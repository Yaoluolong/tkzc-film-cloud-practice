...
const whiteList = ['/login', '/authredirect', '/login/index', '/login/platform', '/login/mall']// no redirect whitelist
    ...
    if (to.path === '/login') {
      ...
      if (!store.getters.name) { // 判断当前用户是否已拉取完user_info信息
        store.dispatch('GetInfo').then(res => { // 拉取user_info
          ...
          //   // console.log('addRoutes', store.getters.addRouters, to)
      ...
      } else if (store.getters.menus.length === 0) { // 判断当前用户是否已拉取完user_info信息
